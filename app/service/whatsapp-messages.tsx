...
function EditorModal({ draft, saving, onChange, onSave, onClose }: { draft: Draft | null; saving: boolean; onChange: (draft: Draft | null) => void; onSave: () => void; onClose: () => void }) {
  ...
  const panResponder = useMemo(() => PanResponder.create({
    ...
    onMoveShouldSetPanResponder: (_event, gesture) => !saving && gesture.dy > 4 && Math.abs(gesture.dy) > Math.abs(gesture.dx),
    onMoveShouldSetPanResponderCapture: (_event, gesture) => !saving && gesture.dy > 4 && Math.abs(gesture.dy) > Math.abs(gesture.dx),
