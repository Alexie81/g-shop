...
import { KeyboardAvoidingView, Modal, Platform, Pressable, RefreshControl, ScrollView, StyleSheet, TextInput, useWindowDimensions, View } from 'react-native';

type Draft = { id?: string; title: string; message: string; sortOrder: string };
const emptyDraft: Draft = { title: '', message: '', sortOrder: '10' };
...
export default function WhatsAppMessagesScreen() {
  ...
  const edit = (message?: WhatsAppMessage) => setDraft(message ? { id: message.id, title: message.title, message: message.message, sortOrder: String(message.sortOrder) } : { ...emptyDraft, sortOrder: String(((state.data?.length ?? 0) + 1) * 10) });
  const save = async () => {
    if (!draft || draft.title.trim().length < 2 || !draft.message.trim()) return showToast('Completează titlul și mesajul.', 'error');
    setSaving(true);
    try {
      const input = { propertyId, title: draft.title.trim(), message: draft.message.trim(), sortOrder: Math.max(0, Math.min(999, Number.parseInt(draft.sortOrder, 10) || 0)) };
    ...
    <View style={styles.root}>
      <LinearGradient onLayout={(event) => setHeroHeight(event.nativeEvent.layout.height)} colors={isDark ? ['#064E3B', '#07A65A'] : ['#075E54', '#25D366']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={[styles.hero, styles.fixedHero, mobile && styles.fixedHeroMobile]}>
        ...
        <View style={styles.heroCopy}>
          ...
          <View style={styles.heroBadge}><Ionicons name="layers-outline" size={14} color="#fff" /><AppText variant="caption" style={styles.heroBadgeText}>{state.data?.length ?? 0} mesaje disponibile</AppText></View>
        </View>
...
function MessageCard({ item, index, canManage, onEdit, onDelete }: { item: WhatsAppMessage; index: number; canManage: boolean; onEdit: () => void; onDelete: () => void }) {
  ...
  return <Card style={styles.messageCard}>
    ...
    <View style={styles.cardTop}>
      ...
      <View style={styles.cardTitle}><AppText variant="heading" numberOfLines={2}>{item.title}</AppText><AppText variant="caption" muted>Ordine {item.sortOrder}</AppText></View>
    ...
    <KeyboardAvoidingView style={[styles.modalOverlay, { backgroundColor: colors.overlay }]} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      ...
      {draft ? <View style={[styles.editor, mobile && styles.editorMobile, { backgroundColor: colors.surface, borderColor: colors.border }]}>
        ...
        <ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false} contentContainerStyle={styles.editorContent}>
          ...
          <View style={styles.field}><AppText variant="label">Ordine afișare</AppText><TextInput value={draft.sortOrder} onChangeText={(sortOrder) => onChange({ ...draft, sortOrder: sortOrder.replace(/\D/g, '').slice(0, 3) })} keyboardType="number-pad" maxLength={3} placeholder="10" placeholderTextColor={colors.textMuted} style={[styles.input, styles.orderInput, { color: colors.text, backgroundColor: colors.input, borderColor: colors.border }]} /></View>
        </ScrollView>
...
const styles = StyleSheet.create({
  ...
  modalOverlay: { flex: 1, justifyContent: 'flex-end', alignItems: 'center' }, editor: { width: '94%', maxWidth: 700, maxHeight: '92%', borderWidth: 1, borderTopLeftRadius: 30, borderTopRightRadius: 30, padding: spacing.xl, gap: spacing.lg }, editorMobile: { width: '100%', paddingHorizontal: spacing.lg }, modalHandle: { width: 46, height: 5, borderRadius: radius.pill, alignSelf: 'center' }, modalHeader: { flexDirection: 'row', alignItems: 'center', gap: spacing.md }, modalIcon: { width: 48, height: 48, borderRadius: radius.md, alignItems: 'center', justifyContent: 'center' }, modalTitle: { minWidth: 0, flex: 1 }, closeButton: { width: 42, height: 42, borderRadius: 21, alignItems: 'center', justifyContent: 'center' }, editorContent: { gap: spacing.lg, paddingBottom: spacing.sm }, field: { gap: spacing.sm }, fieldLabelRow: { flexDirection: 'row', justifyContent: 'space-between', gap: spacing.md }, input: { minHeight: 54, borderWidth: 1, borderRadius: radius.md, paddingHorizontal: spacing.md, fontSize: 15, fontWeight: '600', outlineStyle: 'none' } as never, messageInput: { minHeight: 150, paddingTop: spacing.md, paddingBottom: spacing.md }, orderInput: { width: 110 }, tokens: { gap: spacing.sm }, tokenRow: { gap: spacing.sm, paddingRight: spacing.lg }, token: { height: 38, paddingHorizontal: spacing.md, borderRadius: radius.pill, borderWidth: 1, alignItems: 'center', justifyContent: 'center' }, tokenText: { color: '#10994A', fontWeight: '800' }, modalActions: { flexDirection: 'row', flexWrap: 'wrap', gap: spacing.md }, actionButton: { flex: 1, minWidth: 125 }, actionPrimary: { flex: 1.4, minWidth: 160 },
