...
export default function ClientDetailsScreen() {
  ...
  return <Screen header={<AppHeader title="Detalii client" back onBack={returnToClients} />} refreshing={state.refreshing} onRefresh={() => void state.reload(true)} style={[styles.screenContent, compactLayout && styles.screenContentCompact]}>
    ...
    {tab === 'Detalii' ? <><Details client={client} financials={financials} onOpenFinancials={canViewFinancials ? () => selectTab('Finanțe') : undefined} />{serviceSheet ? <ServiceDocumentsPanel sheet={serviceSheet} onGenerated={() => state.reload(true)} /> : null}{canEditClients ? <ClientLifecycleAction finalized={client.status === 'FINALIZED'} loading={statusSaving} onConfirm={changeClientStatus} /> : null}</> : tab === 'Semnătură' ? <ClientSignaturePanel client={client} canSign={canSignClient} onSign={() => setSignatureOpen(true)} /> : tab === 'QR' ? <ClientQRPanel client={client} /> : tab === 'Finanțe' && financials ? <>
