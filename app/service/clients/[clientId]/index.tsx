...
import { clientRepository, interventionRepository, serviceSheetRepository } from '@/repositories/api-repositories';
import { palette, radius, spacing } from '@/theme/tokens';
import { AuditLog, Client, Intervention, Paginated, ServiceSheet } from '@/types';
...
const tabs = ['Detalii', 'Intervenții', 'Fișe service', 'QR', 'Colaboratori', 'Istoric'] as const; type Tab = typeof tabs[number];
export default function ClientDetailsScreen() {
  const { clientId } = useLocalSearchParams<{ clientId: string }>(); const { colors } = useAppTheme(); const { showToast } = useToast(); const [tab, setTab] = useState<Tab>('Detalii');
  const state = useAsyncData(async () => { const client = await clientRepository.get(clientId); const [sheets, interventions, history] = await Promise.all([serviceSheetRepository.list(client.propertyId), interventionRepository.list(client.propertyId), apiRequest<Paginated<AuditLog>>(`/audit-logs?propertyId=${client.propertyId}&entityId=${client.id}`)]); return { client, sheets: sheets.data.filter((item) => item.clientId === client.id), interventions: interventions.data.filter((item) => item.clientId === client.id), history: history.data }; }, [clientId]);
  if (state.loading) return <Screen header={<AppHeader title="Detalii client" back />}><LoadingState rows={5} /></Screen>; if (state.error || !state.data) return <Screen header={<AppHeader title="Detalii client" back />}><ErrorState message={state.error?.message ?? 'Clientul nu există.'} onRetry={() => void state.reload()} /></Screen>;
  const { client, sheets, interventions, history } = state.data; const contact = async (url: string) => { if (await Linking.canOpenURL(url)) await Linking.openURL(url); else showToast('Acțiunea nu este disponibilă.', 'error'); };
  return <Screen header={<AppHeader title="Detalii client" back />} refreshing={state.refreshing} onRefresh={() => void state.reload(true)}>
    <Card style={styles.profile}><View style={[styles.avatar, { backgroundColor: colors.primary }]}><AppText variant="title" style={{ color: '#fff' }}>{initials(client.firstName, client.lastName)}</AppText></View><View style={styles.profileInfo}><View style={styles.nameRow}><AppText variant="title">{fullName(client)}</AppText><StatusBadge status={client.qr?.status ?? 'NOT_GENERATED'} /></View><AppText muted>{client.phone}{client.email ? ` · ${client.email}` : ''}</AppText><AppText variant="caption" muted>{client.city || 'Localitate nespecificată'} · Client din {formatDate(client.createdAt)}</AppText></View><Button compact variant="outline" icon="create-outline" label="Editează" onPress={() => router.push(`/service/clients/${client.id}/edit`)} /></Card>
    ...
    {tab === 'Detalii' ? <Details client={client} /> : tab === 'QR' ? <ClientQRPanel client={client} onChanged={(next) => state.setData({ ...state.data!, client: next })} /> : tab === 'Fișe service' ? <Sheets items={sheets} clientId={client.id} /> : tab === 'Intervenții' ? <Interventions items={interventions} clientId={client.id} /> : tab === 'Colaboratori' ? <Card style={styles.empty}><Ionicons name="people-circle-outline" size={44} color={colors.primary} /><AppText variant="heading">Colaboratori atribuiți</AppText><AppText muted>{client.collaboratorId ? 'Clientul are un colaborator atribuit. Detaliile comisionului sunt disponibile în modulul Colaboratori.' : 'Nu există colaboratori atribuiți acestui client.'}</AppText><Button compact label="Atribuie colaborator" icon="person-add-outline" onPress={() => router.push('/service/collaborators')} /></Card> : <History items={history} />}
  </Screen>;
}
