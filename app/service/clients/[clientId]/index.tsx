...
export default function ClientDetailsScreen() {
  ...
  const state = useAsyncData(async () => {
    ...
    const [sheets, financials, participants, history] = await Promise.all([
      ...
      isAdmin ? clientRepository.getParticipants(client.id) : Promise.resolve([]),
    ...
    return { client, sheets: sheets.data.filter((item) => item.clientId === client.id), financials, participants, history: history.data };
  ...
  const { client, sheets, financials, participants, history } = state.data;
  ...
  return <Screen header={<AppHeader title="Detalii client" back />} refreshing={state.refreshing} onRefresh={() => void state.reload(true)}>
    ...
    {tab === 'Detalii' ? <Details client={client} /> : tab === 'QR' ? <ClientQRPanel client={client} /> : tab === 'Fișe service' ? <Sheets items={sheets} clientId={client.id} /> : tab === 'Finanțe' && financials ? <ClientFinanceSection
      ...
      onSaveParticipants={isAdmin ? async (userIds) => { const next = await clientRepository.updateParticipants(client.id, userIds); state.setData((current) => current ? { ...current, participants: next } : current); await reloadFinanceHistory(); showToast('Participanții au fost actualizați.', 'success'); } : undefined}
...
const styles = StyleSheet.create({
  profile: { flexDirection: 'row', alignItems: 'center', gap: spacing.md, flexWrap: 'wrap' }, avatar: { width: 62, height: 62, borderRadius: 31, alignItems: 'center', justifyContent: 'center' }, profileInfo: { flex: 1, minWidth: 220, gap: 3 }, nameRow: { flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', gap: spacing.sm }, actions: { flexDirection: 'row', flexWrap: 'wrap', gap: spacing.sm }, tabScroller: { flexGrow: 0, borderBottomWidth: 1 }, tabs: { flexDirection: 'row' }, tab: { paddingVertical: spacing.md, paddingHorizontal: spacing.md, borderBottomWidth: 2, borderBottomColor: 'transparent' }, detailCard: { gap: spacing.lg }, detailGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: spacing.lg }, detailRow: { minWidth: 220, flex: 1, flexDirection: 'row', alignItems: 'center', gap: spacing.md }, smallIcon: { width: 36, height: 36, borderRadius: radius.sm, backgroundColor: '#EAF1FF', alignItems: 'center', justifyContent: 'center' }, empty: { alignItems: 'center', paddingVertical: spacing.xxxl, gap: spacing.md }, sectionHeading: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: spacing.md }, listRow: { flexDirection: 'row', alignItems: 'center', gap: spacing.md, paddingVertical: spacing.md, borderBottomWidth: StyleSheet.hairlineWidth }, timeline: { flexDirection: 'row', gap: spacing.md }, timelineRail: { width: 18, alignItems: 'center' }, timelineDot: { width: 10, height: 10, borderRadius: 5, marginTop: 5 }, line: { flex: 1, width: 2, backgroundColor: '#CBD5E1', marginVertical: 4 }, timelineCopy: { flex: 1, paddingBottom: spacing.lg },
});
