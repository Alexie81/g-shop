...
export default function ServiceSheetsScreen() { const { activeProperty } = useProperty(); const { colors } = useAppTheme(); const state = useAsyncData(() => serviceSheetRepository.list(activeProperty?.id ?? ''), [activeProperty?.id]); return <Screen header={<AppHeader title="Fișe de service" />} refreshing={state.refreshing} onRefresh={() => void state.reload(true)}><View style={styles.heading}><View style={{ flex: 1 }}><AppText variant="title">Fișe de service</AppText><AppText muted>{state.data?.total ?? 0} fișe în proprietatea activă</AppText></View><Button compact label="Fișă nouă" icon="add" onPress={() => router.push('/service/service-sheets/create')} /></View>{state.loading ? <LoadingState rows={5} /> : state.error ? <ErrorState message={state.error.message} onRetry={() => void state.reload()} /> : !state.data?.data.length ? <EmptyState icon="document-text-outline" title="Nicio fișă de service" message="Creează prima fișă direct sau din profilul unui client." action="Creează fișă" onAction={() => router.push('/service/service-sheets/create')} /> : <View style={styles.list}>{state.data.data.map((sheet) => <Pressable key={sheet.id} onPress={() => router.push(`/service/service-sheets/${sheet.id}`)}><Card style={styles.card}><View style={[styles.icon, { backgroundColor: colors.primarySoft }]}><Ionicons name="document-text-outline" size={23} color={colors.primary} /></View><View style={{ flex: 1, gap: 4 }}><View style={styles.row}><AppText variant="heading" style={{ flex: 1 }}>{sheet.number}</AppText><ServiceSheetStatus status={sheet.status} /></View><AppText variant="label">{sheet.equipment}{sheet.brand ? ` · ${sheet.brand}` : ''}{sheet.model ? ` ${sheet.model}` : ''}</AppText><AppText variant="caption" muted numberOfLines={1}>{sheet.client ? `${sheet.client.firstName} ${sheet.client.lastName}` : 'Client'} · {sheet.reportedIssue}</AppText><View style={styles.row}><AppText variant="caption" muted>{formatDate(sheet.receivedAt)}</AppText><AppText variant="label" style={{ color: colors.primary }}>{formatCurrency(sheet.totalCost)}</AppText></View></View><Ionicons name="chevron-forward" size={20} color={colors.textMuted} /></Card></Pressable>)}</View>}</Screen>; }
