      ...
      <View style={styles.stack}>
        {state.loading ? <LoadingState rows={6} /> : state.error ? <ErrorState message={state.error.message} onRetry={() => void state.reload()} /> : sheet ? <>
          ...
          <View style={[styles.metrics, compact && styles.metricsCompact]}>
            ...
            {canViewFinancials ? <Metric label="Rămâne G-Shop" value={money(sheet.gshopNet ?? sheet.receivedAmount, sheet.currencyCode)} color={(sheet.gshopNet ?? sheet.receivedAmount) >= 0 ? palette.purple : palette.danger} /> : null}
          </View>
