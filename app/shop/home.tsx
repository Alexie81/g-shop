...
export default function ShopHomeScreen() {
  ...
  return <Screen header={<AppHeader title="Shop" />}>
    <View style={styles.stack}>
      ...
      {canView ? <View style={[styles.metrics, compact && styles.metricsCompact]}><Metric icon="receipt-outline" label="Fișe emise" value={String(state.data?.total ?? 0)} color={colors.primary} /><Metric icon="cash-outline" label="Total vânzări" value={money(total)} color={palette.success} /><Metric icon="time-outline" label="De încasat" value={money(sheets.reduce((sum, item) => sum + item.remainingDue, 0))} color={palette.warning} /></View> : null}
