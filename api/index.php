...
function ensure_sales_sheets_table(PDO $pdo): void {
    ...
    $pdo->exec("UPDATE sales_sheets SET expenses='[]',expense_total=0,gshop_net=CASE WHEN payment_status='PAID' THEN total_price ELSE LEAST(advance_paid,total_price) END WHERE expenses IS NULL");$ready=true;
}
...
function validated_sales_sheet_payload(array $body): array {
    ...
    $currency=strtoupper(trim((string)($body['currencyCode']??'RON')));if(!preg_match('/^[A-Z]{3}$/',$currency))fail('Moneda nu este validă.',422);$expenses=validated_sales_expenses($body['expenses']??[]);$expenseTotal=round(array_sum(array_column($expenses,'amount')),2);$gshopNet=round($received-$expenseTotal,2);
    return [
