...
require __DIR__ . '/src/sales_documents.php';
...
function ensure_user_deletion_field(): void {
    static $ready = false;
    if ($ready) return;
    $pdo = db();
    $column = $pdo->query("SELECT 1 FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='users' AND column_name='deleted_at' LIMIT 1");
    if (!$column->fetchColumn()) $pdo->exec('ALTER TABLE users ADD COLUMN deleted_at DATETIME NULL AFTER is_active');
    $ready = true;
}
function primary_admin_id(): string {
    $configured = trim(env_value('PRIMARY_ADMIN_ID', '00000000-0000-4000-8000-000000000001'));
    return preg_match('/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i', $configured) ? strtolower($configured) : '00000000-0000-4000-8000-000000000001';
}
function is_primary_admin_id(string $userId): bool { return strtolower($userId) === primary_admin_id(); }
...
function migrate_service_sheet_documents(PDO $pdo): array {
    ...
    $columns = [
        ...
        'warranty_start_at' => 'DATETIME NULL AFTER warranty',
        'warranty_end_at' => 'DATETIME NULL AFTER warranty_start_at',
        'warranty_remediation' => 'VARCHAR(160) NULL AFTER warranty_end_at',
        'storage_after' => 'VARCHAR(120) NULL AFTER warranty_remediation',
...
function ensure_service_warranty_fields(PDO $pdo): void {
    static $ready = false;
    if ($ready) return;
    $columns = [
        'warranty_start_at' => 'DATETIME NULL AFTER warranty',
    ...
    foreach ($columns as $column => $definition) {
        $lookup->execute([$column]);
        if (!$lookup->fetchColumn()) $pdo->exec('ALTER TABLE service_sheets ADD COLUMN ' . $column . ' ' . $definition);
    }
    $ready = true;
}
...
function ensure_service_documents_table(PDO $pdo): void {
    ...
    ensure_service_warranty_fields($pdo);
    $pdo->exec("CREATE TABLE IF NOT EXISTS service_documents (
        id BINARY(16) PRIMARY KEY,
        service_sheet_id BINARY(16) NOT NULL,
        client_id BINARY(16) NOT NULL,
        property_id BINARY(16) NOT NULL,
        type ENUM('INTAKE','FINAL_ESTIMATE','EXIT','WARRANTY') NOT NULL,
    ...
    $typeColumn=$pdo->query("SELECT COLUMN_TYPE FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='service_documents' AND column_name='type' LIMIT 1")->fetchColumn();
    if(is_string($typeColumn)&&stripos($typeColumn,"'WARRANTY'")===false)$pdo->exec("ALTER TABLE service_documents MODIFY type ENUM('INTAKE','FINAL_ESTIMATE','EXIT','WARRANTY') NOT NULL");
    $ready = true;
}
...
function company_details_list(string $propertyId): array {
    ...
    $stmt=db()->prepare(company_select().' pc WHERE pc.is_active=1 AND (pc.id=? OR (pc.property_id=? AND (NOT EXISTS (SELECT 1 FROM property_company_selections other_selection WHERE other_selection.company_id=pc.id AND other_selection.property_id<>?) OR EXISTS (SELECT 1 FROM property_company_selections own_selection WHERE own_selection.company_id=pc.id AND own_selection.property_id=?)))) ORDER BY pc.legal_name,pc.created_at');
    $selectedBinary=$selectedId!==''?uuid_bin($selectedId):uuid_bin('00000000-0000-0000-0000-000000000000');$propertyBinary=uuid_bin($propertyId);$stmt->execute([$selectedBinary,$propertyBinary,$propertyBinary,$propertyBinary]);$items=[];
...
function ensure_gshop_trotinete_property(): void {
    static $ready=false;if($ready)return;$ready=true;
    $pdo=db();ensure_property_companies_table($pdo);migrate_whatsapp_messages($pdo);
    $adminId=primary_admin_id();$propertyId='33333333-3333-4333-8333-333333333333';$companyId='33333333-3333-4333-8333-333333333334';$now=now_utc();
    $existingProperty=$pdo->prepare('SELECT '.uuid_sql('id').' id FROM properties WHERE domain=? LIMIT 1');$existingProperty->execute(['gshop-trotinete.ro']);$existingPropertyId=$existingProperty->fetchColumn();if(is_string($existingPropertyId)&&$existingPropertyId!=='')$propertyId=$existingPropertyId;
    $modules=json_encode(['dashboard','clients','qr','serviceSheets','collaborators','technicians','whatsapp','users','reports','register','audit'],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
    $pdo->beginTransaction();
    try{
        $property=$pdo->prepare('INSERT INTO properties (id,name,domain,type,enabled_modules,is_active,created_at,updated_at,created_by,updated_by) VALUES (?,?,?,?,?,1,?,?,?,?) ON DUPLICATE KEY UPDATE domain=VALUES(domain),type=VALUES(type),enabled_modules=VALUES(enabled_modules),is_active=1');
        $property->execute([uuid_bin($propertyId),'G-Shop Trotinete','gshop-trotinete.ro','SERVICE',$modules,$now,$now,uuid_bin($adminId),uuid_bin($adminId)]);
        $adminExists=$pdo->prepare('SELECT 1 FROM users WHERE id=? AND is_active=1 LIMIT 1');$adminExists->execute([uuid_bin($adminId)]);
        if($adminExists->fetchColumn()){
            $pdo->prepare('INSERT IGNORE INTO user_properties (user_id,property_id) VALUES (?,?)')->execute([uuid_bin($adminId),uuid_bin($propertyId)]);
            $companyExists=$pdo->prepare('SELECT '.uuid_sql('id').' id FROM property_companies WHERE property_id=? AND is_active=1 ORDER BY is_default DESC,created_at LIMIT 1');$companyExists->execute([uuid_bin($propertyId)]);$existingCompanyId=$companyExists->fetchColumn();
            if(!$existingCompanyId){
                $servicePropertyId='11111111-1111-4111-8111-111111111111';$sourceCompany=$pdo->prepare('SELECT '.uuid_sql('pc.id').' id FROM property_companies pc LEFT JOIN property_company_selections pcs ON pcs.property_id=? AND pcs.company_id=pc.id WHERE pc.property_id=? AND pc.is_active=1 ORDER BY (pcs.company_id IS NOT NULL) DESC,pc.is_default DESC,pc.created_at LIMIT 1');$sourceCompany->execute([uuid_bin($servicePropertyId),uuid_bin($servicePropertyId)]);$sourceCompanyId=$sourceCompany->fetchColumn();
                if($sourceCompanyId){
                    $pdo->prepare('INSERT INTO property_companies (id,property_id,is_default,legal_name,tax_id,trade_register_number,vat_payer,address,city,county,postal_code,country,phone,email,website,bank_name,iban,representative_name,representative_role,stamp_path,is_active,created_at,updated_at,created_by,updated_by) SELECT ?,?,1,legal_name,tax_id,trade_register_number,vat_payer,address,city,county,postal_code,country,?,?,?,bank_name,iban,representative_name,representative_role,stamp_path,1,?,?,?,? FROM property_companies WHERE id=? AND is_active=1')->execute([uuid_bin($companyId),uuid_bin($propertyId),'0735046534','contact@gshop-trotinete.ro','gshop-trotinete.ro',$now,$now,uuid_bin($adminId),uuid_bin($adminId),uuid_bin((string)$sourceCompanyId)]);
                }else{
                    $pdo->prepare('INSERT INTO property_companies (id,property_id,is_default,legal_name,country,phone,email,website,is_active,created_at,updated_at,created_by,updated_by) VALUES (?,?,1,?,?,?,?,?,1,?,?,?,?)')->execute([uuid_bin($companyId),uuid_bin($propertyId),'G-Shop Trotinete','România','0735046534','contact@gshop-trotinete.ro','gshop-trotinete.ro',$now,$now,uuid_bin($adminId),uuid_bin($adminId)]);
                }
                $existingCompanyId=$companyId;
            }
            $pdo->prepare('INSERT INTO property_company_selections (property_id,company_id,created_at,updated_at,created_by,updated_by) VALUES (?,?,?,?,?,?) ON DUPLICATE KEY UPDATE company_id=VALUES(company_id),updated_at=VALUES(updated_at),updated_by=VALUES(updated_by)')->execute([uuid_bin($propertyId),uuid_bin((string)$existingCompanyId),$now,$now,uuid_bin($adminId),uuid_bin($adminId)]);
            $messageCount=$pdo->prepare('SELECT COUNT(*) FROM whatsapp_messages WHERE property_id=? AND user_id=?');$messageCount->execute([uuid_bin($propertyId),uuid_bin($adminId)]);
            if((int)$messageCount->fetchColumn()===0){
                $defaults=[['Actualizare reparație','Bună ziua, {prenume}! Vă contactăm din partea {proprietate} cu o actualizare privind trotineta dumneavoastră.',1],['Reparație finalizată','Bună ziua, {prenume}! Trotineta dumneavoastră este pregătită și poate fi ridicată. Vă mulțumim, {proprietate}!',2],['Link status reparație','Bună ziua, {prenume}! Puteți urmări statusul reparației aici: {link_status}',3]];
                ...
                foreach($defaults as$default)$message->execute([uuid_bin(uuid_v4()),uuid_bin($propertyId),uuid_bin($adminId),$default[0],$default[1],$default[2],$now,$now,uuid_bin($adminId),uuid_bin($adminId)]);
            }
        }
        $pdo->commit();
    }catch(Throwable$error){if($pdo->inTransaction())$pdo->rollBack();throw$error;}
}
function company_selected_id(PDO $pdo,string $propertyId,bool $lock=false): ?string {
    $sql='SELECT '.uuid_sql('company_id').' company_id FROM property_company_selections WHERE property_id=? LIMIT 1'.($lock?' FOR UPDATE':'');$stmt=$pdo->prepare($sql);$stmt->execute([uuid_bin($propertyId)]);$id=$stmt->fetchColumn();return is_string($id)&&$id!==''?$id:null;
}
function company_write_selection(PDO $pdo,string $propertyId,string $companyId,array $user,string $now): void {
    $pdo->prepare('INSERT INTO property_company_selections (property_id,company_id,created_at,updated_at,created_by,updated_by) VALUES (?,?,?,?,?,?) ON DUPLICATE KEY UPDATE company_id=VALUES(company_id),updated_at=VALUES(updated_at),updated_by=VALUES(updated_by)')->execute([uuid_bin($propertyId),uuid_bin($companyId),$now,$now,uuid_bin($user['id']),uuid_bin($user['id'])]);
}
function company_remove_stamp_file(?string $relativePath): void {
    if(!$relativePath)return;$root=realpath(__DIR__.'/uploads/stamps');$candidate=realpath(__DIR__.'/'.ltrim($relativePath,'/'));if($root&&$candidate&&str_starts_with($candidate,$root.DIRECTORY_SEPARATOR)&&is_file($candidate))@unlink($candidate);
}
function company_copy_stamp_file(?string $relativePath,string $companyId): ?string {
    if(!$relativePath)return null;$root=realpath(__DIR__.'/uploads/stamps');$source=realpath(__DIR__.'/'.ltrim($relativePath,'/'));if(!$root||!$source||!str_starts_with($source,$root.DIRECTORY_SEPARATOR)||!is_file($source))return null;
    $extension=strtolower((string)pathinfo($source,PATHINFO_EXTENSION));if(!in_array($extension,['png','jpg','jpeg','webp'],true))return null;$target=$root.DIRECTORY_SEPARATOR.$companyId.'.'.$extension;if(!copy($source,$target))throw new RuntimeException('Ștampila firmei nu a putut fi separată pentru acest modul.');@chmod($target,0644);return'uploads/stamps/'.$companyId.'.'.$extension;
}
function company_private_copy_for_property(PDO $pdo,string $companyId,string $propertyId,array $user,bool $selectCopy): array {
    $sourceStmt=$pdo->prepare(company_select().' WHERE id=? AND is_active=1 LIMIT 1 FOR UPDATE');$sourceStmt->execute([uuid_bin($companyId)]);$sourceRow=$sourceStmt->fetch();if(!$sourceRow)fail('Firma nu există.',404);$source=map_company_details($sourceRow,true);
    $selectedId=company_selected_id($pdo,$propertyId,true);if($source['propertyId']!==$propertyId&&$selectedId!==$companyId)fail('Firma nu este configurată pentru acest modul.',409);
    $otherSelections=$pdo->prepare('SELECT property_id FROM property_company_selections WHERE company_id=? AND property_id<>? FOR UPDATE');$otherSelections->execute([uuid_bin($companyId),uuid_bin($propertyId)]);$isShared=$source['propertyId']!==$propertyId||(bool)$otherSelections->fetchColumn();
    if(!$isShared){if($selectCopy)company_write_selection($pdo,$propertyId,$companyId,$user,now_utc());return['id'=>$companyId,'copied'=>false,'copiedStampPath'=>null,'wasSelected'=>$selectedId===$companyId,'source'=>$source];}
    $copyId=uuid_v4();$copiedStampPath=company_copy_stamp_file($source['stampPath']??null,$copyId);$now=now_utc();$columns=['id','property_id','is_default','legal_name','tax_id','trade_register_number','vat_payer','address','city','county','postal_code','country','phone','email','website','bank_name','iban','representative_name','representative_role','stamp_path','is_active','created_at','updated_at','created_by','updated_by'];
    $copy=$pdo->prepare('INSERT INTO property_companies ('.implode(',',$columns).') SELECT ?,?,0,legal_name,tax_id,trade_register_number,vat_payer,address,city,county,postal_code,country,phone,email,website,bank_name,iban,representative_name,representative_role,?,1,?,?,?,? FROM property_companies WHERE id=? AND is_active=1');
    try{$copy->execute([uuid_bin($copyId),uuid_bin($propertyId),$copiedStampPath,$now,$now,uuid_bin($user['id']),uuid_bin($user['id']),uuid_bin($companyId)]);if($copy->rowCount()!==1)throw new RuntimeException('Firma nu a putut fi separată pentru acest modul.');if($selectCopy||$selectedId===$companyId)company_write_selection($pdo,$propertyId,$copyId,$user,$now);}catch(Throwable$error){company_remove_stamp_file($copiedStampPath);throw$error;}
    return['id'=>$copyId,'copied'=>true,'copiedStampPath'=>$copiedStampPath,'wasSelected'=>$selectedId===$companyId,'source'=>$source];
}
...
function company_prepare_shop_sales_sync(PDO $pdo,string $propertyId): bool {
    if((string)(property_record($propertyId)['type']??'')!=='SHOP')return false;ensure_sales_sheets_table($pdo);ensure_sales_documents_table($pdo);return true;
}
function company_sync_shop_sales_sheets(PDO $pdo,string $propertyId,string $sourceCompanyId,string $targetCompanyId,array $company,array $user): array {
    $stmt=$pdo->prepare('SELECT '.uuid_sql('id').' id FROM sales_sheets WHERE property_id=? AND is_active=1 AND (company_id=? OR company_id=?) ORDER BY created_at,id FOR UPDATE');$stmt->execute([uuid_bin($propertyId),uuid_bin($sourceCompanyId),uuid_bin($targetCompanyId)]);$ids=array_values(array_filter(array_map(fn($row)=>(string)($row['id']??''),$stmt->fetchAll())));if(!$ids)return[];
    $snapshot=json_encode(company_sheet_snapshot($company),JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES|JSON_THROW_ON_ERROR);$now=now_utc();$update=$pdo->prepare('UPDATE sales_sheets SET company_id=?,company_snapshot=?,updated_at=?,updated_by=? WHERE id=? AND property_id=? AND is_active=1');foreach($ids as$id)$update->execute([uuid_bin($targetCompanyId),$snapshot,$now,uuid_bin($user['id']),uuid_bin($id),uuid_bin($propertyId)]);return$ids;
}
function company_refresh_shop_sales_pdfs(array $sheetIds,array $user): array {
    $refreshed=[];$failed=[];foreach(array_values(array_unique($sheetIds))as$sheetId){invalidate_sales_dossier_files($sheetId);try{generate_sales_sheet_record($sheetId,$user);$refreshed[]=$sheetId;}catch(Throwable$error){$failed[]=$sheetId;error_log('[G-Shop company PDF sync] '.$sheetId.': '.$error->getMessage());}}return['refreshed'=>$refreshed,'failed'=>$failed];
}
...
function ensure_sales_sheets_table(PDO $pdo): void {
    ...
    $columns=$pdo->query("SELECT column_name FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='sales_sheets' AND column_name IN ('expenses','expense_total','gshop_net','payment_status')")->fetchAll(PDO::FETCH_COLUMN);$paymentStatusMissing=!in_array('payment_status',$columns,true);
    if(!in_array('expenses',$columns,true))$pdo->exec("ALTER TABLE sales_sheets ADD COLUMN expenses LONGTEXT NULL AFTER notes");
...
function sales_sheet_select(): string {
    return 'SELECT '.uuid_sql('ss.id').' id,'.uuid_sql('ss.property_id').' property_id,'.uuid_sql('ss.company_id').' company_id,ss.company_snapshot,ss.number,ss.document_at,ss.customer_name,ss.customer_phone,ss.customer_email,ss.delivery_address,ss.customer_notes,ss.product_name,ss.product_code,ss.serial_number,ss.quantity,ss.warranty,ss.payment_method,ss.delivery_mode,ss.product_unit_price,ss.product_price,ss.delivery_price,ss.total_price,ss.advance_paid,ss.remaining_due,ss.payment_status,ss.due_at,ss.currency_code,ss.notes,ss.expenses,ss.expense_total,ss.gshop_net,ss.signature_path,ss.signed_at,ss.file_path,ss.file_sha256,ss.generated_at,ss.status,ss.is_active,ss.created_at,ss.updated_at,'.uuid_sql('ss.created_by').' created_by,'.uuid_sql('ss.updated_by').' updated_by,pc.legal_name company_name FROM sales_sheets ss LEFT JOIN property_companies pc ON pc.id=ss.company_id';
}
function map_sales_sheet(array $row, bool $includePaths=false): array {
    $item=entity_base($row);foreach(['quantity','productUnitPrice','productPrice','deliveryPrice','totalPrice','advancePaid','remainingDue','expenseTotal','gshopNet']as$key)$item[$key]=round((float)($item[$key]??0),2);
    $expenses=json_decode((string)($row['expenses']??'[]'),true);$item['expenses']=[];if(is_array($expenses)){foreach($expenses as$expense){if(!is_array($expense))continue;$item['expenses'][]=['name'=>(string)($expense['name']??''),'quantity'=>max(0.01,round((float)($expense['quantity']??1),2)),'amount'=>max(0,round((float)($expense['amount']??0),2))];}}$item['expenseTotal']=sales_expense_total($item['expenses']);
    $item['paymentStatus']=in_array(($item['paymentStatus']??''),['UNPAID','PAID'],true)?$item['paymentStatus']:'UNPAID';$item['receivedAmount']=round($item['paymentStatus']==='PAID'?max($item['advancePaid'],$item['totalPrice']):max(0,$item['advancePaid']),2);$item['remainingDue']=round(max(0,$item['totalPrice']-$item['receivedAmount']),2);$item['gshopNet']=round($item['totalPrice']-$item['expenseTotal'],2);
...
function sales_sheet_for_user(array $sheet,array $user): array {if(empty($user['isPrimaryAdmin'])&&!in_array('financials.view',$user['permissions'],true))foreach(['expenses','expenseTotal','gshopNet']as$key)unset($sheet[$key]);return$sheet;}
...
function locked_sales_sheet_row(PDO $pdo,string $id): array {
    ensure_sales_sheets_table($pdo);$stmt=$pdo->prepare(sales_sheet_select().' WHERE ss.id=? AND ss.is_active=1 LIMIT 1 FOR UPDATE');$stmt->execute([uuid_bin(validated_uuid($id,'Fișa de vânzare'))]);$row=$stmt->fetch();if(!$row)fail('Fișa de vânzare nu există.',404);return$row;
}
...
function validated_sales_expenses(mixed $value): array {
    if($value===null||$value==='')return[];if(!is_array($value))fail('Lista cheltuielilor nu este validă.',422);if(count($value)>40)fail('Poți adăuga maximum 40 de cheltuieli pe o fișă.',422);$expenses=[];
    foreach($value as$line){if(!is_array($line))fail('O cheltuială nu este validă.',422);$name=sales_sheet_text($line['name']??null,'Denumirea cheltuielii',160);$quantityInput=$line['quantity']??1;$quantity=(is_string($quantityInput)&&trim($quantityInput)==='')||$quantityInput===null?1.0:sales_sheet_number_value($quantityInput,'Cantitatea cheltuielii',999999,true);$amount=sales_sheet_number_value($line['amount']??0,'Valoarea unitară a cheltuielii',999999999.99);if($name===null&&$amount<=0)continue;if($name===null)fail('Completează denumirea fiecărei cheltuieli.',422);if($amount<=0)fail('Valoarea unitară a fiecărei cheltuieli trebuie să fie mai mare decât zero.',422);$expenses[]=['name'=>$name,'quantity'=>$quantity,'amount'=>$amount];}
    return$expenses;
}
function sales_expense_total(array $expenses): float {$total=0.0;foreach($expenses as$expense){if(!is_array($expense))continue;$total+=max(0,(float)($expense['quantity']??1))*max(0,(float)($expense['amount']??0));}return round($total,2);}
function validated_sales_sheet_payload(array $body): array {
    $quantityInput=$body['quantity']??1;$quantity=(is_string($quantityInput)&&trim($quantityInput)==='')||$quantityInput===null||(is_numeric($quantityInput)&&(float)$quantityInput<=0)?1.0:sales_sheet_number_value($quantityInput,'Cantitatea',9999,true);$unit=sales_sheet_number_value($body['productUnitPrice']??0,'Prețul produsului',999999999.99);$delivery=sales_sheet_number_value($body['deliveryPrice']??0,'Manopera / serviciile',999999999.99);$advance=sales_sheet_number_value($body['advancePaid']??0,'Banii încasați',999999999.99);
    $productPrice=round($quantity*$unit,2);$total=round($productPrice+$delivery,2);$paymentStatus=strtoupper(trim((string)($body['paymentStatus']??'UNPAID')));if(!in_array($paymentStatus,['UNPAID','PAID'],true))fail('Statusul plății nu este valid.',422);if($paymentStatus==='UNPAID'&&$advance>$total)fail('Banii încasați nu pot depăși prețul total.',422);$received=$paymentStatus==='PAID'?max($total,$advance):$advance;$remaining=round(max(0,$total-$received),2);
    ...
    $currency=strtoupper(trim((string)($body['currencyCode']??'RON')));if(!preg_match('/^[A-Z]{3}$/',$currency))fail('Moneda nu este validă.',422);$expenses=validated_sales_expenses($body['expenses']??[]);$expenseTotal=sales_expense_total($expenses);$gshopNet=round($total-$expenseTotal,2);
    return [
        'documentAt'=>sales_sheet_db_date($body['documentAt']??now_utc(),'Data fișei',true),'customerName'=>sales_sheet_text($body['customerName']??null,'Numele și prenumele / Firma',160,true),'customerPhone'=>sales_sheet_text($body['customerPhone']??null,'Telefonul clientului',30,true),
        ...
        'productName'=>sales_sheet_text($body['productName']??null,'Produsul / modelul',220)??'','productCode'=>sales_sheet_text($body['productCode']??null,'Codul produsului',80),'serialNumber'=>sales_sheet_text($body['serialNumber']??null,'Seria / IMEI',120),'quantity'=>$quantity,'warranty'=>sales_sheet_text($body['warranty']??null,'Garanția',100),
        'paymentMethod'=>$payment,'deliveryMode'=>$deliveryMode,'productUnitPrice'=>$unit,'productPrice'=>$productPrice,'deliveryPrice'=>$delivery,'totalPrice'=>$total,'advancePaid'=>$received,'remainingDue'=>$remaining,'paymentStatus'=>$paymentStatus,'dueAt'=>sales_sheet_db_date($body['dueAt']??null,'Data scadenței'),'currencyCode'=>$currency,'notes'=>sales_sheet_text($body['notes']??null,'Observațiile',1600),'expenses'=>json_encode($expenses,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),'expenseTotal'=>$expenseTotal,'gshopNet'=>$gshopNet,
    ];
}
...
function generate_sales_sheet_record(string $sheetId,array $user,int $attempt=0): array {
    ...
    try{
        $pdo->beginTransaction();$locked=locked_sales_sheet_row($pdo,$sheetId);
        if(sales_document_sheet_sync_fingerprint(map_sales_sheet($locked,true))!==$sourceFingerprint)throw new SalesDocumentSyncConflict('Fișa s-a modificat în timpul generării PDF-ului.');
        $pdo->prepare('UPDATE sales_sheets SET file_path=?,file_sha256=?,generated_at=?,updated_at=?,updated_by=? WHERE id=?')->execute([$rendered['filePath'],$rendered['sha256'],$generatedAt,now_utc(),uuid_bin($user['id']),uuid_bin($sheetId)]);$pdo->commit();
    }catch(Throwable$error){if($pdo->inTransaction())$pdo->rollBack();remove_sales_sheet_pdf_file($rendered['filePath']??null);if($error instanceof SalesDocumentSyncConflict&&$attempt<2)return generate_sales_sheet_record($sheetId,$user,$attempt+1);if($error instanceof SalesDocumentSyncConflict)fail('Fișa s-a modificat între timp. Încearcă din nou.',409);throw$error;}
    if($previousPath!==''&&$previousPath!==$rendered['filePath'])remove_sales_sheet_pdf_file($previousPath);refresh_existing_sales_documents_safe($sheetId,$user);return get_sales_sheet($sheetId);
}
...
function service_document_definitions(): array {
    return [
        ...
        'WARRANTY'=>['label'=>'Certificat de garanție','slug'=>'warranty','prefix'=>'GAR'],
    ];
}
...
function map_service_document(array $row, ?string $publicToken = null, bool $public = false): array {
    ...
    $snapshot=json_decode((string)($row['snapshot_json']??''),true);if(is_array($snapshot)){$snapshotSheet=is_array($snapshot['sheet']??null)?$snapshot['sheet']:[];if($item['type']==='FINAL_ESTIMATE')$item['technicalAssessment']=(string)($snapshotSheet['technicalAssessment']??'');if($item['type']==='INTAKE')$item['estimatedCosts']=service_document_estimated_costs_from_snapshot($snapshot);if($item['type']==='WARRANTY'){$warranty=is_array($snapshot['warranty']??null)?$snapshot['warranty']:[];$item['warrantyPeriod']=(string)($warranty['period']??'');$item['warrantyStartAt']=$warranty['startAt']??null;$item['warrantyEndAt']=$warranty['endAt']??null;$item['warrantyRemediation']=(string)($warranty['remediation']??'');}}
...
function regenerate_existing_service_documents(string $sheetId, array $user): void {
    ensure_service_documents_table(db());$sheet=get_sheet($sheetId);ensure_property((string)$sheet['propertyId'],$user);$signaturePath=service_document_signature_path($sheetId);$company=company_for_service_sheet($sheet);$stmt=db()->prepare(service_document_select().' WHERE d.service_sheet_id=? AND d.is_active=1 ORDER BY FIELD(d.type,\'INTAKE\',\'FINAL_ESTIMATE\',\'EXIT\',\'WARRANTY\')');$stmt->execute([uuid_bin($sheetId)]);$rows=$stmt->fetchAll();if(!$rows)return;require_once __DIR__.'/src/service_document_pdf.php';
...
function rebalance_service_document_items(array $source, float $displayedTarget, float $directTarget, string $fallbackName): array {
    $items=service_document_items($source,'');$displayedTarget=round(max(0,$displayedTarget),2);$directTarget=round(max(0,$directTarget),2);
    if(!$items){if($displayedTarget<=0&&$directTarget<=0)return[];$items=[['name'=>$fallbackName,'quantity'=>1.0,'unitPrice'=>0.0,'totalPrice'=>0.0,'directCost'=>0.0]];}
    $displayedBase=array_sum(array_column($items,'totalPrice'));$directBase=array_sum(array_column($items,'directCost'));$displayedAssigned=0.0;$directAssigned=0.0;$last=count($items)-1;
    foreach($items as$index=>&$item){
        $quantity=max(.01,(float)$item['quantity']);
        $displayed=$index===$last?round($displayedTarget-$displayedAssigned,2):round($displayedTarget*($displayedBase>0?(float)$item['totalPrice']/$displayedBase:($index===0?1:0)),2);
        $direct=$index===$last?round($directTarget-$directAssigned,2):round($directTarget*($directBase>0?(float)$item['directCost']/$directBase:($index===0?1:0)),2);
        $displayed=max(0,$displayed);$direct=max(0,$direct);$item['unitPrice']=round($displayed/$quantity,2);$item['totalPrice']=round($quantity*$item['unitPrice'],2);$item['directCost']=$direct;$displayedAssigned+=$displayed;$directAssigned+=$direct;
    }
    unset($item);return$items;
}
function regenerate_client_final_estimates_from_financials(string $clientId, array $user): int {
    $client=get_client($clientId);$bundle=client_financial_bundle($client);$financial=$bundle['financials'];$stmt=db()->prepare('SELECT '.uuid_sql('id').' id FROM service_sheets WHERE client_id=? AND is_active=1');$stmt->execute([uuid_bin($clientId)]);$count=0;
    foreach($stmt->fetchAll()as$sheetRow){
        $sheetId=(string)$sheetRow['id'];$row=service_document_existing_row($sheetId,'FINAL_ESTIMATE');if(!$row)continue;$snapshot=json_decode((string)($row['snapshot_json']??''),true);if(!is_array($snapshot))throw new RuntimeException('Snapshotul devizului final nu este valid.');$sheet=get_sheet($sheetId);
        $partsSource=is_array($snapshot['parts']??null)?$snapshot['parts']:[];$laborSource=is_array($snapshot['labor']??null)?$snapshot['labor']:[];
        $parts=rebalance_service_document_items($partsSource,(float)$financial['displayedPartsCost'],(float)$financial['actualPartsCost'],trim((string)($sheet['partsUsed']??''))?:'Piese și componente');
        $labor=rebalance_service_document_items($laborSource,(float)$financial['displayedLaborCost'],0.0,trim((string)($sheet['workPerformed']??''))?:'Manoperă service');
        generate_service_document_record($sheetId,'FINAL_ESTIMATE',['parts'=>$parts,'labor'=>$labor],$user,false);$count++;
    }
    if($count>0)audit_log('FINAL_ESTIMATE_REGENERATED_FROM_FINANCIALS','service_documents','Devizul final a fost regenerat automat din datele financiare curente','Client',$clientId,$client['propertyId'],null,['documentsRegenerated'=>$count],$user);return$count;
}
...
function generate_service_document_record(string $sheetId, string $type, array $body, array $user, bool $syncFinalFinancials = true): array {
    ...
    $itemsRequestFinancialSync=!array_key_exists('syncFinancialsFromItems',$body)||!empty($body['syncFinancialsFromItems']);
    if($syncFinalFinancials&&$itemsRequestFinancialSync&&$type==='FINAL_ESTIMATE'&&(array_key_exists('parts',$body)||array_key_exists('labor',$body)))$sheet=sync_final_document_financials($sheet,$parts,$labor,$user);
    ...
    $defaultDocumentAt=$type==='INTAKE'?gmdate('Y-m-d H:i:s',strtotime((string)$sheet['receivedAt'])):(in_array($type,['EXIT','WARRANTY'],true)&&!empty($sheet['completedAt'])?gmdate('Y-m-d H:i:s',strtotime((string)$sheet['completedAt'])):$now);$documentAt=service_document_db_date($body['documentAt']??null,$existing['document_at']??$defaultDocumentAt);
    $defaultAgreement=$type==='INTAKE'&&!empty($sheet['signedAt'])?gmdate('Y-m-d H:i:s',strtotime((string)$sheet['signedAt'])):(in_array($type,['EXIT','WARRANTY'],true)?$documentAt:$now);$agreementAt=array_key_exists('agreementAt',$body)?service_document_optional_db_date($body['agreementAt']):service_document_db_date($existing['agreement_at']??null,$defaultAgreement);
    ...
    $existingSnapshotSheet=is_array($existingSnapshot['sheet']??null)?$existingSnapshot['sheet']:[];$technicalAssessment=company_detail_text($body['technicalAssessment']??$existingSnapshotSheet['technicalAssessment']??$sheet['technicalAssessment']??null,'Diagnosticarea',2000);$defectCause=company_detail_text($body['defectCause']??$existing['defect_cause']??null,'Cauza defectului',40);$finalNotes=company_detail_text($body['finalNotes']??$existing['final_notes']??null,'Observațiile finale',2000);
    $existingWarranty=is_array($existingSnapshot['warranty']??null)?$existingSnapshot['warranty']:[];$warrantyPeriod=company_detail_text($body['warrantyPeriod']??$existingWarranty['period']??$sheet['warranty']??null,'Perioada garanției',120);$warrantyStartAt=array_key_exists('warrantyStartAt',$body)?service_document_optional_db_date($body['warrantyStartAt']):service_document_optional_db_date($existingWarranty['startAt']??$sheet['warrantyStartAt']??$sheet['completedAt']??null);$warrantyEndAt=array_key_exists('warrantyEndAt',$body)?service_document_optional_db_date($body['warrantyEndAt']):service_document_optional_db_date($existingWarranty['endAt']??$sheet['warrantyEndAt']??null);$warrantyRemediation=company_detail_text($body['warrantyRemediation']??$existingWarranty['remediation']??$sheet['warrantyRemediation']??null,'Termenul de remediere',160);
    $intake=service_document_reference($sheetId,'INTAKE');$estimate=service_document_reference($sheetId,'FINAL_ESTIMATE');$exitReference=service_document_reference($sheetId,'EXIT');$signaturePath=service_document_signature_path($sheetId);
    $snapshotSheet=array_merge($sheet,['technicalAssessment'=>$technicalAssessment,'estimatedRepairDays'=>$estimatedDays,'estimatedTotal'=>$bundle['summary']['totalDue'],'finalNotes'=>$finalNotes,'defectCause'=>$defectCause,'finalAgreementAt'=>iso_date($agreementAt),'deliveredAt'=>$type==='EXIT'?iso_date($documentAt):($sheet['completedAt']??null)]);$snapshotFinancials=$bundle['financials'];$snapshotSummary=$bundle['summary'];$snapshotEstimate=$estimate??['number'=>$type==='FINAL_ESTIMATE'?$number:'','date'=>$type==='FINAL_ESTIMATE'?iso_date($documentAt):'','total'=>$bundle['summary']['totalDue'],'remaining'=>$bundle['summary']['remainingDue']];
    ...
    $snapshot=['company'=>$company,'client'=>$client,'sheet'=>$snapshotSheet,'intake'=>$intake??['number'=>$type==='INTAKE'?$number:'','date'=>$type==='INTAKE'?iso_date($documentAt):''],'estimate'=>$snapshotEstimate,'parts'=>$parts,'labor'=>$labor,'financials'=>$snapshotFinancials,'summary'=>$snapshotSummary,'agreement'=>['status'=>$agreementStatus,'date'=>iso_date($agreementAt)],'exit'=>$exitReference??['number'=>$type==='EXIT'?$number:'','date'=>$type==='EXIT'?iso_date($documentAt):'','productState'=>$productState],'warranty'=>['number'=>$type==='WARRANTY'?$number:'','date'=>$type==='WARRANTY'?iso_date($documentAt):'','period'=>$warrantyPeriod,'startAt'=>iso_date($warrantyStartAt),'endAt'=>iso_date($warrantyEndAt),'remediation'=>$warrantyRemediation,'contact'=>implode(' · ',array_values(array_filter([$company['phone']??null,$company['email']??null])))]];
...
try {
    ...
    if ($method === 'GET' && $path === '/app-update') {
        ...
        respond(['platform'=>'android','latestVersion'=>env_value('APP_ANDROID_VERSION','1.0.0'),'latestBuildNumber'=>max(0,(int)env_value('APP_ANDROID_BUILD_NUMBER','0')),'downloadUrl'=>env_value('APP_ANDROID_DOWNLOAD_URL',''),'releaseNotes'=>$notes,'publishedAt'=>env_value('APP_ANDROID_PUBLISHED_AT',''),'mandatory'=>env_value('APP_ANDROID_MANDATORY','0')==='1']);
    }

    if ($method === 'POST' && $path === '/push/devices') {
        $user=current_user();$body=json_body();$propertyId=validated_uuid((string)($body['propertyId']??''),'Proprietatea');ensure_property($propertyId,$user);$device=gshop_register_push_device($user,$propertyId,$body);respond($device,201);
    }
    ...
    if ($method === 'GET' && $path === '/properties') {
        $user = current_user();ensure_gshop_trotinete_property(); $sql = 'SELECT ' . uuid_sql('p.id') . ' id,p.name,p.domain,p.type,p.enabled_modules,p.is_active,p.created_at,p.updated_at,' . uuid_sql('p.created_by') . ' created_by,' . uuid_sql('p.updated_by') . ' updated_by FROM properties p';
    ...
    if ($method === 'PUT' && path_match('/companies/{id}/default',$path,$params)) {
        $user=require_permission('settings.manage');if($user['role']!=='ADMIN')fail('Doar administratorul poate selecta firma activă.',403);$id=validated_uuid($params['id'],'Firma');$body=json_body();$propertyId=validated_uuid((string)($body['propertyId']??''),'Proprietatea');ensure_property($propertyId,$user);$before=company_details_by_id($id);$pdo=db();ensure_property_companies_table($pdo);$syncShop=company_prepare_shop_sales_sync($pdo,$propertyId);$copyResult=null;$salesSheetIds=[];
        try{$pdo->beginTransaction();$copyResult=company_private_copy_for_property($pdo,$id,$propertyId,$user,true);$targetId=(string)$copyResult['id'];if($syncShop&&!empty($copyResult['copied'])){$targetCompany=company_details_by_id($targetId,$propertyId,true);$salesSheetIds=company_sync_shop_sales_sheets($pdo,$propertyId,$id,$targetId,$targetCompany,$user);}$pdo->commit();}catch(Throwable$error){if($pdo->inTransaction())$pdo->rollBack();if(!empty($copyResult['copied']))company_remove_stamp_file($copyResult['copiedStampPath']??null);throw$error;}
        $pdfSync=company_refresh_shop_sales_pdfs($salesSheetIds,$user);$after=company_details_by_id($targetId,$propertyId);$after['isDefault']=true;audit_log(!empty($copyResult['copied'])?'COMPANY_COPIED_AND_SELECTED':'DEFAULT_COMPANY_CHANGED','settings','Firma activă este acum '.$after['legalName'],'Company',$targetId,$propertyId,['companyId'=>$id,'isDefault'=>false],['companyId'=>$targetId,'isDefault'=>true,'isolated'=>!empty($copyResult['copied']),'salesSheetsUpdated'=>count($salesSheetIds),'pdfRefreshFailed'=>count($pdfSync['failed'])],$user);respond($after);
    }
    if ($method === 'PUT' && path_match('/companies/{id}',$path,$params)) {
        $user=require_permission('settings.manage');if($user['role']!=='ADMIN')fail('Doar administratorul poate modifica firmele.',403);$id=validated_uuid($params['id'],'Firma');$body=json_body();$propertyId=validated_uuid((string)($body['propertyId']??''),'Proprietatea');ensure_property($propertyId,$user);$before=company_details_by_id($id);$values=validated_company_payload($body);$now=now_utc();$sets=['legal_name','tax_id','trade_register_number','vat_payer','address','city','county','postal_code','country','phone','email','website','bank_name','iban','representative_name','representative_role'];$assignments=array_map(fn($column)=>$column.'=?',$sets);$pdo=db();ensure_property_companies_table($pdo);$syncShop=company_prepare_shop_sales_sync($pdo,$propertyId);$copyResult=null;$salesSheetIds=[];
        try{$pdo->beginTransaction();$copyResult=company_private_copy_for_property($pdo,$id,$propertyId,$user,false);$targetId=(string)$copyResult['id'];$pdo->prepare('UPDATE property_companies SET '.implode(',',$assignments).',updated_at=?,updated_by=? WHERE id=? AND property_id=?')->execute([...$values,$now,uuid_bin($user['id']),uuid_bin($targetId),uuid_bin($propertyId)]);if($syncShop){$targetCompany=company_details_by_id($targetId,$propertyId,true);$salesSheetIds=company_sync_shop_sales_sheets($pdo,$propertyId,$id,$targetId,$targetCompany,$user);}$pdo->commit();}catch(Throwable$error){if($pdo->inTransaction())$pdo->rollBack();if(!empty($copyResult['copied']))company_remove_stamp_file($copyResult['copiedStampPath']??null);throw$error;}
        $pdfSync=company_refresh_shop_sales_pdfs($salesSheetIds,$user);$after=company_details_by_id($targetId,$propertyId);$after['isDefault']=company_selected_id(db(),$propertyId)===$targetId;audit_log(!empty($copyResult['copied'])?'COMPANY_COPIED_AND_UPDATED':'COMPANY_UPDATED','settings','Firmă actualizată: '.$after['legalName'],'Company',$targetId,$propertyId,company_details_snapshot($before),[...company_details_snapshot($after),'isolated'=>!empty($copyResult['copied']),'sourceCompanyId'=>$id,'salesSheetsUpdated'=>count($salesSheetIds),'pdfRefreshFailed'=>count($pdfSync['failed'])],$user);respond($after);
    }
    if ($method === 'POST' && path_match('/companies/{id}/stamp',$path,$params)) {
        $user=require_permission('settings.manage');if($user['role']!=='ADMIN')fail('Doar administratorul poate modifica ștampila.',403);$id=validated_uuid($params['id'],'Firma');$body=json_body();$propertyId=validated_uuid((string)($body['propertyId']??''),'Proprietatea');ensure_property($propertyId,$user);$before=company_details_by_id($id);$data=(string)($body['stamp']??'');if(!preg_match('#^data:image/(png|jpeg|webp);base64,(.+)$#',$data,$match))fail('Formatul ștampilei nu este valid.',422);$binary=base64_decode($match[2],true);if($binary===false||strlen($binary)<100||strlen($binary)>2500000)fail('Imaginea ștampilei este invalidă sau prea mare.',422);$extensions=['png'=>'png','jpeg'=>'jpg','webp'=>'webp'];$extension=$extensions[$match[1]];$directory=__DIR__.'/uploads/stamps';if(!is_dir($directory)&&!mkdir($directory,0755,true)&&!is_dir($directory))throw new RuntimeException('Directorul pentru ștampile nu poate fi creat.');$pdo=db();ensure_property_companies_table($pdo);$syncShop=company_prepare_shop_sales_sync($pdo,$propertyId);$copyResult=null;$salesSheetIds=[];$pathValue=null;$previousStampPath=null;
        try{$pdo->beginTransaction();$copyResult=company_private_copy_for_property($pdo,$id,$propertyId,$user,false);$targetId=(string)$copyResult['id'];$targetBefore=company_details_by_id($targetId,$propertyId,true);$previousStampPath=$targetBefore['stampPath']??null;$filename=$targetId.'.'.strtolower(substr(str_replace('-','',uuid_v4()),0,12)).'.'.$extension;$pathValue='uploads/stamps/'.$filename;if(file_put_contents($directory.'/'.$filename,$binary,LOCK_EX)===false)throw new RuntimeException('Ștampila nu poate fi salvată.');@chmod($directory.'/'.$filename,0644);$now=now_utc();$pdo->prepare('UPDATE property_companies SET stamp_path=?,updated_at=?,updated_by=? WHERE id=? AND property_id=?')->execute([$pathValue,$now,uuid_bin($user['id']),uuid_bin($targetId),uuid_bin($propertyId)]);if($syncShop){$targetCompany=company_details_by_id($targetId,$propertyId,true);$salesSheetIds=company_sync_shop_sales_sheets($pdo,$propertyId,$id,$targetId,$targetCompany,$user);}$pdo->commit();}catch(Throwable$error){if($pdo->inTransaction())$pdo->rollBack();company_remove_stamp_file($pathValue);if(!empty($copyResult['copied']))company_remove_stamp_file($copyResult['copiedStampPath']??null);throw$error;}
        if($previousStampPath&&$previousStampPath!==$pathValue)company_remove_stamp_file($previousStampPath);$pdfSync=company_refresh_shop_sales_pdfs($salesSheetIds,$user);$after=company_details_by_id($targetId,$propertyId);$after['isDefault']=company_selected_id(db(),$propertyId)===$targetId;audit_log(!empty($copyResult['copied'])?'COMPANY_COPIED_AND_STAMP_UPDATED':'COMPANY_STAMP_UPDATED','settings','Ștampila firmei a fost actualizată','Company',$targetId,$propertyId,['hasStamp'=>!empty($before['stampUrl']),'sourceCompanyId'=>$id],['hasStamp'=>true,'isolated'=>!empty($copyResult['copied']),'salesSheetsUpdated'=>count($salesSheetIds),'pdfRefreshFailed'=>count($pdfSync['failed'])],$user);respond($after);
    }
    if ($method === 'DELETE' && path_match('/companies/{id}/stamp',$path,$params)) {
        $user=require_permission('settings.manage');if($user['role']!=='ADMIN')fail('Doar administratorul poate elimina ștampila.',403);$id=validated_uuid($params['id'],'Firma');$propertyId=validated_uuid((string)($_GET['propertyId']??''),'Proprietatea');ensure_property($propertyId,$user);$before=company_details_by_id($id);$pdo=db();ensure_property_companies_table($pdo);$syncShop=company_prepare_shop_sales_sync($pdo,$propertyId);$copyResult=null;$salesSheetIds=[];$previousStampPath=null;
        try{$pdo->beginTransaction();$copyResult=company_private_copy_for_property($pdo,$id,$propertyId,$user,false);$targetId=(string)$copyResult['id'];$targetBefore=company_details_by_id($targetId,$propertyId,true);$previousStampPath=$targetBefore['stampPath']??null;$pdo->prepare('UPDATE property_companies SET stamp_path=NULL,updated_at=?,updated_by=? WHERE id=? AND property_id=?')->execute([now_utc(),uuid_bin($user['id']),uuid_bin($targetId),uuid_bin($propertyId)]);if($syncShop){$targetCompany=company_details_by_id($targetId,$propertyId,true);$salesSheetIds=company_sync_shop_sales_sheets($pdo,$propertyId,$id,$targetId,$targetCompany,$user);}$pdo->commit();}catch(Throwable$error){if($pdo->inTransaction())$pdo->rollBack();if(!empty($copyResult['copied']))company_remove_stamp_file($copyResult['copiedStampPath']??null);throw$error;}
        company_remove_stamp_file($previousStampPath);$pdfSync=company_refresh_shop_sales_pdfs($salesSheetIds,$user);$after=company_details_by_id($targetId,$propertyId);$after['isDefault']=company_selected_id(db(),$propertyId)===$targetId;audit_log(!empty($copyResult['copied'])?'COMPANY_COPIED_AND_STAMP_DELETED':'COMPANY_STAMP_DELETED','settings','Ștampila firmei a fost eliminată','Company',$targetId,$propertyId,['hasStamp'=>!empty($before['stampUrl']),'sourceCompanyId'=>$id],['hasStamp'=>false,'isolated'=>!empty($copyResult['copied']),'salesSheetsUpdated'=>count($salesSheetIds),'pdfRefreshFailed'=>count($pdfSync['failed'])],$user);respond($after);
    }
    ...
    if ($method === 'GET' && $path === '/dashboard') {
        ...
        $financeStmt=$pdo->prepare($financeSql);$financeStmt->execute([$p]);$financeRevenueRon=0.0;$financeHoldRon=0.0;$financeNetRon=0.0;$financeInternalRon=0.0;$financeWaiting=0;$financeCollaboratorPaidRon=0.0;$financeCollaboratorOnHoldRon=0.0;
        foreach($financeStmt->fetchAll()as$row){
            ...
            $financeRevenueRon+=$received*$rate;$financeHoldRon+=$remaining*$rate;$financeInternalRon+=$internal*$rate;$financeNetRon+=round($received-$internal-(float)$row['paid_total'],2)*$rate;if($remaining>0.004)$financeWaiting++;
        }
    ...
    if ($method === 'PUT' && path_match('/clients/{id}/financials',$path,$params)) {
        ...
        if(!financial_has_data($next)&&$expenseCount===0){
            ...
            $pdo->commit();regenerate_client_final_estimates_from_financials($client['id'],$user);
        ...
        if($before['persisted']&&$beforeSnapshot===$nextSnapshot){$expenses=client_expenses($client['id']);$beforeCollaborator=client_collaborator_finance($client,$before,financial_summary($client,$before,$expenses));sync_service_sheet_financials_from_client($pdo,$client,$before,$expenses,$user);$syncResult=sync_client_commission($pdo,$client,$before,$expenses,$user,false);$pdo->commit();regenerate_client_final_estimates_from_financials($client['id'],$user);if($syncResult['changed'])audit_log('CLIENT_COMMISSION_RECALCULATED','commissions','Comisionul colaboratorului a fost recalculat din finanțele clientului','Client',$client['id'],$client['propertyId'],$beforeCollaborator,client_financial_bundle($client)['collaborator'],$user);respond(client_financial_bundle($client));}
        $now=now_utc();
        if($before['persisted']){
        ...
        $after=client_financial_record($client);$expenses=client_expenses($client['id']);$beforeCollaborator=client_collaborator_finance($client,$before,financial_summary($client,$before,$expenses));sync_service_sheet_financials_from_client($pdo,$client,$after,$expenses,$user);$syncResult=sync_client_commission($pdo,$client,$after,$expenses,$user,false);$pdo->commit();regenerate_client_final_estimates_from_financials($client['id'],$user);
    ...
    if($method==='POST'&&path_match('/sales-sheets/{id}/documents/{type}',$path,$params)){
        $user=require_permission('sales_sheets.update');require_permission('financials.view');$sheet=get_sales_sheet($params['id']);ensure_property($sheet['propertyId'],$user);$type=validated_sales_document_type($params['type']);$document=generate_sales_document_record($sheet['id'],$type,json_body(),$user);$afterSheet=get_sales_sheet($sheet['id']);audit_log('SALES_DOCUMENT_GENERATED','sales_documents',$document['label'].' generat pentru '.$sheet['number'],'SalesDocument',$document['id']??null,$sheet['propertyId'],['type'=>$type,'sheetTotalPrice'=>$sheet['totalPrice'],'receivedAmount'=>$sheet['receivedAmount'],'expenseTotal'=>$sheet['expenseTotal']??null],['type'=>$type,'number'=>$document['number']??null,'sheetTotalPrice'=>$afterSheet['totalPrice'],'receivedAmount'=>$afterSheet['receivedAmount'],'expenseTotal'=>$afterSheet['expenseTotal']??null],$user);respond($document,201);
    }
    if($method==='DELETE'&&path_match('/sales-sheets/{id}/documents/{type}',$path,$params)){
        $user=require_permission('sales_sheets.update');$sheet=get_sales_sheet($params['id']);ensure_property($sheet['propertyId'],$user);$type=validated_sales_document_type($params['type']);$deleted=delete_sales_document_record($sheet['id'],$type,$user);if(!$deleted)fail('Documentul nu există.',404);audit_log('SALES_DOCUMENT_DELETED','sales_documents','Documente eliminate din dosarul '.$sheet['number'],'SalesSheet',$sheet['id'],$sheet['propertyId'],['type'=>$type],['deletedTypes'=>$deleted],$user);respond(['deleted'=>true,'types'=>$deleted]);
    }
    if($method==='POST'&&path_match('/sales-sheets/{id}/pdf',$path,$params)){$user=require_permission('sales_sheets.view');$before=get_sales_sheet($params['id']);ensure_property($before['propertyId'],$user);$after=generate_sales_sheet_record($before['id'],$user);audit_log('SALES_SHEET_PDF_GENERATED','sales_sheets','PDF generat pentru '.$after['number'],'SalesSheet',$after['id'],$after['propertyId'],null,['fileName'=>basename((string)parse_url((string)$after['pdfUrl'],PHP_URL_PATH))],$user);respond(sales_sheet_for_user($after,$user));}
    if($method==='POST'&&$path==='/sales-sheets'){
        ...
        audit_log('SALES_SHEET_CREATED','sales_sheets','Fișă de vânzare emisă: '.$after['number'],'SalesSheet',$id,$propertyId,null,$after,$user);respond(sales_sheet_for_user($after,$user),201);
    }
    if($method==='PUT'&&path_match('/sales-sheets/{id}',$path,$params)){
        $user=require_permission('sales_sheets.update');$body=json_body();$canViewFinancials=user_has_permission($user,'financials.view');
        if(!$canViewFinancials&&!empty($body['expenses']))fail('Nu ai permisiunea de a modifica cheltuieli interne.',403);
        $pdo=db();ensure_sales_documents_table($pdo);$pdo->beginTransaction();
        try{
            $row=locked_sales_sheet_row($pdo,$params['id']);$before=map_sales_sheet($row);ensure_property($before['propertyId'],$user);
            if(!$canViewFinancials)$body['expenses']=$before['expenses']??[];
            $hasAcceptedEstimate=sales_document_has_accepted_final_estimate($pdo,$before['id'],true);
            if($hasAcceptedEstimate){
                $body['quantity']=1;$body['productUnitPrice']=$before['productPrice'];$body['deliveryPrice']=$before['deliveryPrice'];$body['advancePaid']=$before['receivedAmount'];$body['paymentStatus']=$before['paymentStatus'];$body['currencyCode']=$before['currencyCode'];
            }
            $values=validated_sales_sheet_payload($body);
            if($hasAcceptedEstimate){
                foreach(['quantity','productUnitPrice','productPrice','deliveryPrice','totalPrice','advancePaid','remainingDue','paymentStatus','currencyCode']as$key)$values[$key]=$key==='advancePaid'?round((float)$before['receivedAmount'],2):$before[$key];
                $values['gshopNet']=round((float)$values['totalPrice']-(float)$values['expenseTotal'],2);
            }
            $columns=['document_at','customer_name','customer_phone','customer_email','delivery_address','customer_notes','product_name','product_code','serial_number','quantity','warranty','payment_method','delivery_mode','product_unit_price','product_price','delivery_price','total_price','advance_paid','remaining_due','payment_status','due_at','currency_code','notes','expenses','expense_total','gshop_net'];
            $args=array_values($values);$assignments=array_map(fn($column)=>$column.'=?',$columns);$args[]=now_utc();$args[]=uuid_bin($user['id']);$args[]=uuid_bin($before['id']);
            $pdo->prepare('UPDATE sales_sheets SET '.implode(',',$assignments).',updated_at=?,updated_by=? WHERE id=?')->execute($args);$pdo->commit();
        }catch(Throwable$error){if($pdo->inTransaction())$pdo->rollBack();throw$error;}
        $after=generate_sales_sheet_record($before['id'],$user);audit_log('SALES_SHEET_UPDATED','sales_sheets','Fișă de vânzare actualizată: '.$after['number'],'SalesSheet',$after['id'],$after['propertyId'],$before,$after,$user);respond(sales_sheet_for_user($after,$user));
    }
    if($method==='PUT'&&path_match('/sales-sheets/{id}/expenses',$path,$params)){
        $user=require_permission('sales_sheets.update');require_permission('financials.view');$expenses=validated_sales_expenses(json_body()['expenses']??[]);$expenseTotal=sales_expense_total($expenses);$pdo=db();ensure_sales_sheets_table($pdo);$pdo->beginTransaction();
        try{
            $row=locked_sales_sheet_row($pdo,$params['id']);$before=map_sales_sheet($row);ensure_property($before['propertyId'],$user);$gshopNet=round((float)$before['totalPrice']-$expenseTotal,2);$now=now_utc();
            $pdo->prepare('UPDATE sales_sheets SET expenses=?,expense_total=?,gshop_net=?,updated_at=?,updated_by=? WHERE id=?')->execute([json_encode($expenses,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES|JSON_THROW_ON_ERROR),$expenseTotal,$gshopNet,$now,uuid_bin($user['id']),uuid_bin($before['id'])]);$pdo->commit();
        }catch(Throwable$error){if($pdo->inTransaction())$pdo->rollBack();throw$error;}
        $after=get_sales_sheet($before['id']);audit_log('SALES_SHEET_EXPENSES_UPDATED','sales_sheets','Cheltuieli actualizate pe '.$after['number'],'SalesSheet',$after['id'],$after['propertyId'],['expenses'=>$before['expenses']??[],'expenseTotal'=>$before['expenseTotal']??0],['expenses'=>$after['expenses'],'expenseTotal'=>$after['expenseTotal']],$user);respond($after);
    }
    if($method==='PUT'&&path_match('/sales-sheets/{id}/payment',$path,$params)){
        $user=require_permission('sales_sheets.update');$body=json_body();$pdo=db();ensure_sales_sheets_table($pdo);$pdo->beginTransaction();
        try{
            $row=locked_sales_sheet_row($pdo,$params['id']);$before=map_sales_sheet($row);ensure_property($before['propertyId'],$user);$paymentStatus=strtoupper(trim((string)($body['paymentStatus']??$before['paymentStatus']??'UNPAID')));
            if(!in_array($paymentStatus,['UNPAID','PAID'],true))fail('Statusul plății nu este valid.',422);$advance=sales_sheet_number_value($body['advancePaid']??$before['receivedAmount']??0,'Banii încasați',999999999.99);$total=(float)$before['totalPrice'];
            if($paymentStatus==='UNPAID'&&$advance>$total)fail('Banii încasați nu pot depăși prețul total.',422);$received=$paymentStatus==='PAID'?max($total,$advance):$advance;$remaining=round(max(0,$total-$received),2);$gshopNet=round($total-(float)($before['expenseTotal']??0),2);$now=now_utc();
            $pdo->prepare('UPDATE sales_sheets SET advance_paid=?,remaining_due=?,payment_status=?,gshop_net=?,updated_at=?,updated_by=? WHERE id=?')->execute([$received,$remaining,$paymentStatus,$gshopNet,$now,uuid_bin($user['id']),uuid_bin($before['id'])]);$pdo->commit();
        }catch(Throwable$error){if($pdo->inTransaction())$pdo->rollBack();throw$error;}
        $after=generate_sales_sheet_record($before['id'],$user);audit_log('SALES_SHEET_PAYMENT_UPDATED','sales_sheets','Plată actualizată pe '.$after['number'],'SalesSheet',$after['id'],$after['propertyId'],['paymentStatus'=>$before['paymentStatus'],'receivedAmount'=>$before['receivedAmount']],['paymentStatus'=>$after['paymentStatus'],'receivedAmount'=>$after['receivedAmount']],$user);respond(sales_sheet_for_user($after,$user));
    }
    ...
    if ($method==='PUT'&&path_match('/users/{id}',$path,$params)) {
        ...
        audit_log('USER_ROLE_UPDATED','users','Rol actualizat pentru @'.$after['username'],'User',$after['id'],$after['propertyIds'][0]??($before['propertyIds'][0]??null),$before,$after,$admin);respond($after);
    }
    if ($method==='PUT'&&path_match('/users/{id}/permissions',$path,$params)) {
        ...
        $before=user_record($params['id'],false);ensure_primary_admin_editable($admin,$before);$propertyIds=array_key_exists('propertyIds',$body)?$body['propertyIds']:$before['propertyIds'];
        ...
        $after=user_record($params['id'],false);audit_log('USER_ACCESS_UPDATED','users','Acces și permisiuni actualizate pentru @'.$after['username'],'User',$after['id'],$after['propertyIds'][0]??($before['propertyIds'][0]??null),$before,$after,$admin);respond($after);
    }
    if ($method==='PUT'&&path_match('/users/{id}/status',$path,$params)) {
        $admin=require_permission('users.manage');$body=json_body();if(!array_key_exists('isActive',$body)||!is_bool($body['isActive']))fail('Starea utilizatorului este invalidă.',422);$isActive=(bool)$body['isActive'];if($params['id']===$admin['id']&&!$isActive)fail('Nu îți poți dezactiva propriul cont.',422);$before=user_record($params['id'],false);ensure_primary_admin_editable($admin,$before);
        if((bool)$before['isActive']===$isActive)respond($before);$now=now_utc();db()->prepare('UPDATE users SET is_active=?,updated_at=?,updated_by=? WHERE id=? AND deleted_at IS NULL')->execute([$isActive?1:0,$now,uuid_bin($admin['id']),uuid_bin($params['id'])]);if(!$isActive)db()->prepare('UPDATE refresh_sessions SET revoked_at=? WHERE user_id=? AND revoked_at IS NULL')->execute([$now,uuid_bin($params['id'])]);$after=user_record($params['id'],false);
        audit_log($isActive?'USER_REACTIVATED':'USER_DEACTIVATED','users',($isActive?'Utilizator reactivat: @':'Utilizator dezactivat: @').$after['username'],'User',$after['id'],$after['propertyIds'][0]??null,$before,$after,$admin);respond($after);
    }
    if ($method==='PUT'&&path_match('/users/{id}/password',$path,$params)) { $admin=require_permission('users.manage');$body=json_body();$password=(string)($body['password']??'');if(strlen($password)<8)fail('Parola trebuie să aibă minimum 8 caractere.',422);$target=user_record($params['id'],false);ensure_primary_admin_editable($admin,$target,true);db()->prepare('UPDATE users SET password_hash=?,updated_at=?,updated_by=? WHERE id=?')->execute([password_hash($password,PASSWORD_DEFAULT),now_utc(),uuid_bin($admin['id']),uuid_bin($params['id'])]);db()->prepare('UPDATE refresh_sessions SET revoked_at=? WHERE user_id=? AND revoked_at IS NULL')->execute([now_utc(),uuid_bin($params['id'])]);audit_log('USER_PASSWORD_RESET','users','Parolă resetată pentru @'.$target['username'],'User',$target['id'],$target['propertyIds'][0]??null,null,null,$admin);respond(['changed'=>true]); }
    if ($method==='DELETE'&&path_match('/users/{id}',$path,$params)) { $admin=require_permission('users.manage');if($params['id']===$admin['id'])fail('Nu îți poți șterge propriul cont.',422);$target=user_record($params['id'],false);ensure_primary_admin_editable($admin,$target);$now=now_utc();$pdo=db();$pdo->beginTransaction();try{$pdo->prepare('UPDATE users SET is_active=0,deleted_at=?,updated_at=?,updated_by=? WHERE id=?')->execute([$now,$now,uuid_bin($admin['id']),uuid_bin($params['id'])]);$pdo->prepare('DELETE FROM user_properties WHERE user_id=?')->execute([uuid_bin($params['id'])]);$pdo->prepare('UPDATE refresh_sessions SET revoked_at=? WHERE user_id=? AND revoked_at IS NULL')->execute([$now,uuid_bin($params['id'])]);$pdo->commit();}catch(Throwable$error){if($pdo->inTransaction())$pdo->rollBack();throw$error;}audit_log('USER_DELETED','users','Utilizator șters: @'.$target['username'],'User',$target['id'],$target['propertyIds'][0]??null,$target,['deleted'=>true,'isActive'=>false],$admin);respond(['deleted'=>true]); }
    ...
    if ($method==='GET'&&$path==='/reports') {
        ...
        $financialInternal=(float)$scalar("SELECT COALESCE(SUM((COALESCE(cf.actual_parts_cost,0)+COALESCE(ex.total_expenses,0))*COALESCE(cf.exchange_rate_to_ron,1)),0) FROM clients c JOIN client_financials cf ON cf.client_id=c.id LEFT JOIN (SELECT client_id,SUM(amount) total_expenses FROM client_expenses GROUP BY client_id) ex ON ex.client_id=c.id WHERE c.property_id=? AND c.is_active=1",[$p]);
        $legacyInternal=(float)$scalar("SELECT COALESCE(SUM(s.direct_costs),0) FROM service_sheets s JOIN clients c ON c.id=s.client_id LEFT JOIN client_financials cf ON cf.client_id=c.id WHERE s.property_id=? AND c.is_active=1 AND cf.client_id IS NULL AND NOT EXISTS(SELECT 1 FROM client_expenses e WHERE e.client_id=c.id) AND s.is_active=1 AND s.status<>'CANCELLED'",[$p]);
        $paidCommissionExpenses=(float)$scalar("SELECT COALESCE(SUM(co.commission_value*COALESCE(cf.exchange_rate_to_ron,1)),0) FROM commissions co JOIN clients c ON c.id=co.client_id LEFT JOIN client_financials cf ON cf.client_id=c.id WHERE co.property_id=? AND c.is_active=1 AND co.is_active=1 AND co.status='PAID' AND co.paid_at IS NOT NULL",[$p]);$totalExpenses=$financialInternal+$legacyInternal+$paidCommissionExpenses;
        ...
        $periodRevenueRon=0.0;$periodHoldRon=0.0;$periodNetRon=0.0;$periodInternalRon=0.0;$periodWaiting=0;$periodCollaboratorPaidRon=0.0;$periodCollaboratorOnHoldRon=0.0;
        foreach($periodFinanceStmt->fetchAll()as$row){
            ...
            $periodRevenueRon+=$received*$rate;$periodHoldRon+=$remaining*$rate;$periodInternalRon+=$internal*$rate;$periodNetRon+=round($received-$internal-(float)$row['paid_total'],2)*$rate;if($remaining>0.004)$periodWaiting++;
        }
        ...
        $periodCollaboratorPaid=$periodCollaboratorPaidRon+(float)$periodLegacyCommission['paid'];$periodCollaboratorOnHold=$periodCollaboratorOnHoldRon+(float)$periodLegacyCommission['on_hold'];$periodCollaboratorTotal=$periodCollaboratorPaid+$periodCollaboratorOnHold;$periodExpenses=$periodInternalRon+(float)$periodLegacy['direct_costs']+$periodCollaboratorPaid;
        ...
        $periodMetrics=['clientsTotal'=>$periodClientsTotal,'clientsWaiting'=>$periodClientsWaiting,'gshopNet'=>round($periodGshopNet,2),'revenueOnHold'=>round($periodRevenueOnHold,2),'totalRevenue'=>round($periodRevenue,2),'totalExpenses'=>round($periodExpenses,2),'collaboratorTotal'=>round($periodCollaboratorTotal,2),'collaboratorPaid'=>round($periodCollaboratorPaid,2),'collaboratorOnHold'=>round($periodCollaboratorOnHold,2)];
