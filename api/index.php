...
function ensure_sales_sheets_table(PDO $pdo): void {
    ...
    $columns=$pdo->query("SELECT column_name FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='sales_sheets' AND column_name IN ('expenses','expense_total','gshop_net')")->fetchAll(PDO::FETCH_COLUMN);
    ...
    $pdo->exec("UPDATE sales_sheets SET expenses='[]',expense_total=0,gshop_net=advance_paid WHERE expenses IS NULL");$ready=true;
}
...
function validated_sales_sheet_payload(array $body): array {
    ...
    $currency=strtoupper(trim((string)($body['currencyCode']??'RON')));if(!preg_match('/^[A-Z]{3}$/',$currency))fail('Moneda nu este validă.',422);$expenses=validated_sales_expenses($body['expenses']??[]);$expenseTotal=round(array_sum(array_column($expenses,'amount')),2);$gshopNet=round($advance-$expenseTotal,2);
    return [
