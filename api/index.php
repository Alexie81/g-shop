...
try {
    ...
    if ($method === 'GET' && $path === '/dashboard') {
        ...
        $totalExpenses=(float)$scalar('SELECT COALESCE(SUM(e.amount*COALESCE(cf.exchange_rate_to_ron,1)),0) FROM client_expenses e JOIN clients c ON c.id=e.client_id LEFT JOIN client_financials cf ON cf.client_id=c.id WHERE c.property_id=? AND c.is_active=1',[$p]);
    ...
    if ($method==='GET'&&$path==='/reports') {
        ...
        $periodExpenses=(float)$scalar('SELECT COALESCE(SUM(e.amount*COALESCE(cf.exchange_rate_to_ron,1)),0) FROM client_expenses e JOIN clients c ON c.id=e.client_id LEFT JOIN client_financials cf ON cf.client_id=c.id WHERE c.property_id=? AND c.is_active=1 AND e.created_at>=? AND e.created_at<?',[$p,$startSql,$endSql]);
