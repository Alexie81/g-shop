...
function generate_service_document_record(string $sheetId, string $type, array $body, array $user, bool $syncFinalFinancials = true): array {
    ...
    if($type==='FINAL_ESTIMATE'&&!$intake)fail('Creează mai întâi fișa de intrare.',409,['code'=>'INTAKE_REQUIRED','requiredType'=>'INTAKE']);
    if($type==='EXIT'&&!$intake)fail('Creează mai întâi fișa de intrare.',409,['code'=>'INTAKE_REQUIRED','requiredType'=>'INTAKE']);
    if($type==='EXIT'&&!$estimate)fail('Creează mai întâi devizul final.',409,['code'=>'FINAL_ESTIMATE_REQUIRED','requiredType'=>'FINAL_ESTIMATE']);
    if($type==='WARRANTY'&&!$intake)fail('Creează mai întâi fișa de intrare.',409,['code'=>'INTAKE_REQUIRED','requiredType'=>'INTAKE']);
    ...
    if($type==='WARRANTY'&&!$exitReference)fail('Creează mai întâi fișa de ieșire.',409,['code'=>'EXIT_REQUIRED','requiredType'=>'EXIT']);
