...
try {
    ...
    if ($method==='GET'&&$path==='/users') { $user=require_permission('users.view');ensure_user_deletion_field();$propertyId=(string)($_GET['propertyId']??'');ensure_property($propertyId,$user);$select='SELECT '.uuid_sql('u.id').' id,u.username,u.first_name,u.last_name,u.email,u.phone,u.role,u.permissions,u.is_active,u.last_login_at,u.created_at,u.updated_at,'.uuid_sql('u.created_by').' created_by,'.uuid_sql('u.updated_by').' updated_by FROM users u';$args=[];if($user['role']!=='ADMIN'){$select.=' JOIN user_properties up ON up.user_id=u.id WHERE u.deleted_at IS NULL AND up.property_id=?';$args[] = uuid_bin($propertyId);}else{$select.=' WHERE u.deleted_at IS NULL';}$select.=' ORDER BY u.is_active DESC,u.first_name,u.last_name';$stmt=db()->prepare($select);$stmt->execute($args);$data=[];foreach($stmt->fetchAll()as$row){$item=entity_base($row);$item['permissions']=json_decode((string)$row['permissions'],true)?:[];$item['propertyIds']=[];$pstmt=db()->prepare('SELECT '.uuid_sql('property_id').' id FROM user_properties WHERE user_id=?');$pstmt->execute([uuid_bin($item['id'])]);$item['propertyIds']=array_column($pstmt->fetchAll(),'id');$data[]=$item;}respond($data); }
    ...
    if ($method==='PUT'&&path_match('/users/{id}',$path,$params)) {
        ...
        $before=user_record($params['id'],false);db()->prepare('UPDATE users SET role=?,updated_at=?,updated_by=? WHERE id=? AND deleted_at IS NULL')->execute([$role,now_utc(),uuid_bin($admin['id']),uuid_bin($params['id'])]);$after=user_record($params['id'],false);
    ...
    if ($method==='PUT'&&path_match('/users/{id}/permissions',$path,$params)) {
        ...
        $before=user_record($params['id'],false);$propertyIds=array_key_exists('propertyIds',$body)?$body['propertyIds']:$before['propertyIds'];
    ...
    if ($method==='PUT'&&path_match('/users/{id}/status',$path,$params)) {
        $admin=require_permission('users.manage');$body=json_body();if(!array_key_exists('isActive',$body)||!is_bool($body['isActive']))fail('Starea utilizatorului este invalidă.',422);$isActive=(bool)$body['isActive'];if($params['id']===$admin['id']&&!$isActive)fail('Nu îți poți dezactiva propriul cont.',422);$before=user_record($params['id'],false);
