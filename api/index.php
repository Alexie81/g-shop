...
function commission_amount(float $netValue, string $type, float $rateOrAmount): float {
    $netValue = max(0, $netValue);
    $rateOrAmount = max(0, $rateOrAmount);
    return round($type === 'PERCENT_NET' ? $netValue * $rateOrAmount / 100 : $rateOrAmount, 2);
}
...
try {
    ...
    if ($method === 'GET' && $path === '/dashboard') {
        ...
        $qrGenerated=(int)$scalar("SELECT COUNT(*) FROM client_qr WHERE property_id=? AND is_active=1 AND status IN ('GENERATED','SENT')",[$p]);$qrUsed=(int)$scalar("SELECT COUNT(*) FROM client_qr WHERE property_id=? AND is_active=1 AND status='USED'",[$p]);$qrUnused=max(0,$clients-$qrGenerated-$qrUsed);
        $revenue=(float)$scalar("SELECT COALESCE(SUM(total_cost),0) FROM service_sheets WHERE property_id=? AND is_active=1 AND status<>'CANCELLED'",[$p]);$commissions=(float)$scalar("SELECT COALESCE(SUM(commission_value),0) FROM commissions WHERE property_id=? AND is_active=1 AND status<>'CANCELLED'",[$p]);$payments=(float)$scalar("SELECT COALESCE(SUM(commission_value),0) FROM commissions WHERE property_id=? AND is_active=1 AND status='PAID' AND paid_at IS NOT NULL",[$p]);
        respond(['clientsTotal'=>$clients,'clientsNew'=>$clientsNew,'serviceSheetsOpen'=>$open,'serviceSheetsInProgress'=>$progress,'serviceSheetsCompleted'=>$completed,'usersActive'=>$users,'collaboratorsActive'=>$collabs,'qrGenerated'=>$qrGenerated,'qrUsed'=>$qrUsed,'qrUnused'=>$qrUnused,'estimatedRevenue'=>$revenue,'collaboratorCommissions'=>$commissions,'collaboratorPayments'=>$payments]);
    }
    ...
    if ($method === 'GET' && $path === '/clients') {
        ...
        $sql=client_select().' WHERE '.implode(' AND ',$where).' ORDER BY c.updated_at DESC LIMIT '.$pageSize.' OFFSET '.(($page-1)*$pageSize);$stmt=db()->prepare($sql);$stmt->execute($args);$data=array_map('map_client',$stmt->fetchAll());
    ...
    if ($method === 'GET' && path_match('/clients/{id}', $path, $params)) { $user=require_permission('clients.view');$client=get_client($params['id']);ensure_property($client['propertyId'],$user);respond($client); }
    if ($method === 'POST' && $path === '/clients') {
        ...
        $collaboratorId=trim((string)($body['collaboratorId']??''));$commissionType=null;$commissionValue=null;
        if($collaboratorId!==''){
            $collaborator=collaborator_for_property($collaboratorId,$propertyId);$commissionType=(string)($body['commissionType']??$collaborator['defaultCommissionType']);$commissionValue=$body['commissionValue']??$collaborator['defaultCommissionValue'];
            if(!in_array($commissionType,['PERCENT_NET','FIXED'],true))fail('Tipul comisionului nu este valid.',422);
            if(!is_numeric($commissionValue)||(float)$commissionValue<0||($commissionType==='PERCENT_NET'&&(float)$commissionValue>100))fail('Valoarea comisionului nu este validă.',422);
            $commissionValue=(float)$commissionValue;
        }
        $id=uuid_v4();$now=now_utc();$stmt=db()->prepare('INSERT INTO clients (id,property_id,first_name,last_name,phone,secondary_phone,email,address,city,county,postal_code,notes,status,collaborator_id,commission_type,commission_value,is_active,created_at,updated_at,created_by,updated_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,1,?,?,?,?)');
        $stmt->execute([uuid_bin($id),uuid_bin($propertyId),trim($body['firstName']),trim($body['lastName']),trim($body['phone']),$body['secondaryPhone']??null,$body['email']??null,$body['address']??null,$body['city']??null,$body['county']??null,$body['postalCode']??null,$body['notes']??null,$body['status']??'NEW',$collaboratorId!==''?uuid_bin($collaboratorId):null,$commissionType,$commissionValue,$now,$now,uuid_bin($user['id']),uuid_bin($user['id'])]);
        $created=get_client($id);audit_log('CLIENT_CREATED','clients','Client creat: '.trim($body['firstName'].' '.$body['lastName']),'Client',$id,$propertyId,null,$created,$user);respond($created,201);
    }
    if ($method === 'PUT' && path_match('/clients/{id}', $path, $params)) {
        ...
        if(array_key_exists('collaboratorId',$body)&&trim((string)($body['collaboratorId']??''))!==''){
            $nextCollaboratorId=trim((string)$body['collaboratorId']);$collaborator=collaborator_for_property($nextCollaboratorId,$before['propertyId']);$changedCollaborator=($before['collaboratorId']??null)!==$nextCollaboratorId;
        ...
        if(count(array_intersect(array_keys($body),['collaboratorId','commissionType','commissionValue']))>0){
            ...
            if($effectiveCollaboratorId!==''){$effectiveType=(string)($body['commissionType']??$before['commissionType']??'');$effectiveValue=$body['commissionValue']??$before['commissionValue'];if(!in_array($effectiveType,['PERCENT_NET','FIXED'],true)||!is_numeric($effectiveValue)||(float)$effectiveValue<0||($effectiveType==='PERCENT_NET'&&(float)$effectiveValue>100))fail('Setările comisionului nu sunt valide.',422);}
        }
        ...
        foreach($map as$key=>$column)if(array_key_exists($key,$body)){
            if($key==='commissionType'&&!in_array($body[$key],['PERCENT_NET','FIXED'],true))fail('Tipul comisionului nu este valid.',422);
            if($key==='commissionValue'&&(!is_numeric($body[$key])||(float)$body[$key]<0||(($body['commissionType']??$before['commissionType'])==='PERCENT_NET'&&(float)$body[$key]>100)))fail('Valoarea comisionului nu este validă.',422);
        ...
        $sets[]='updated_at=?';$args[]=now_utc();$sets[]='updated_by=?';$args[]=uuid_bin($user['id']);$args[]=uuid_bin($params['id']);
        db()->prepare('UPDATE clients SET '.implode(',',$sets).' WHERE id=?')->execute($args);
        $after=get_client($params['id']);audit_log('CLIENT_UPDATED','clients','Client actualizat: '.$after['firstName'].' '.$after['lastName'],'Client',$params['id'],$after['propertyId'],$before,$after,$user);respond($after);
    }
    if ($method === 'POST' && path_match('/clients/{id}/qr', $path, $params)) { $user=require_permission('qr.generate');$client=get_client($params['id']);ensure_property($client['propertyId'],$user);$pdo=db();$pdo->beginTransaction();try{$now=now_utc();$pdo->prepare("UPDATE client_qr SET status='INVALIDATED',is_active=0,invalidated_at=?,updated_at=?,updated_by=? WHERE client_id=? AND is_active=1")->execute([$now,$now,uuid_bin($user['id']),uuid_bin($client['id'])]);$id=uuid_v4();$token=uuid_v4();$pdo->prepare("INSERT INTO client_qr (id,client_id,property_id,token,status,generated_at,expires_at,generated_by,is_active,created_at,updated_at,created_by,updated_by) VALUES (?,?,?,?, 'GENERATED',?,?,?,1,?,?,?,?)")->execute([uuid_bin($id),uuid_bin($client['id']),uuid_bin($client['propertyId']),uuid_bin($token),$now,gmdate('Y-m-d H:i:s',time()+31536000),uuid_bin($user['id']),$now,$now,uuid_bin($user['id']),uuid_bin($user['id'])]);$pdo->commit();audit_log($client['qr']?'QR_REGENERATED':'QR_GENERATED','qr',($client['qr']?'QR regenerat':'QR generat').' pentru '.$client['firstName'].' '.$client['lastName'],'ClientQR',$id,$client['propertyId'],$client['qr']??null,['status'=>'GENERATED'],$user);respond(get_client($client['id']));}catch(Throwable$e){$pdo->rollBack();throw$e;} }
    if ($method === 'POST' && path_match('/clients/{id}/qr/share', $path, $params)) { $user=require_permission('qr.share');$client=get_client($params['id']);if(empty($client['qr']))fail('Clientul nu are un QR activ.',422);$body=json_body();$methodName=strtoupper((string)($body['method']??'NATIVE'));$allowed=['WHATSAPP','EMAIL','SMS','COPY','NATIVE'];if(!in_array($methodName,$allowed,true))$methodName='NATIVE';$now=now_utc();db()->prepare("UPDATE client_qr SET status=IF(status='USED','USED','SENT'),sent_at=?,updated_at=?,updated_by=? WHERE id=?")->execute([$now,$now,uuid_bin($user['id']),uuid_bin($client['qr']['id'])]);db()->prepare("INSERT INTO qr_shares (id,qr_id,client_id,property_id,user_id,method,status,sent_at) VALUES (?,?,?,?,?,?, 'SENT',?)")->execute([uuid_bin(uuid_v4()),uuid_bin($client['qr']['id']),uuid_bin($client['id']),uuid_bin($client['propertyId']),uuid_bin($user['id']),$methodName,$now]);audit_log('QR_SHARED','qr','Cod QR trimis prin '.$methodName,'ClientQR',$client['qr']['id'],$client['propertyId'],null,['method'=>$methodName],$user);respond(['shared'=>true]); }
    if ($method === 'POST' && path_match('/clients/{id}/qr/use', $path, $params)) { $user=require_permission('qr.generate');$client=get_client($params['id']);if(empty($client['qr']))fail('QR inexistent.',404);$now=now_utc();db()->prepare("UPDATE client_qr SET status='USED',used_at=?,updated_at=?,updated_by=? WHERE id=?")->execute([$now,$now,uuid_bin($user['id']),uuid_bin($client['qr']['id'])]);audit_log('QR_MARKED_USED','qr','Cod QR marcat ca folosit','ClientQR',$client['qr']['id'],$client['propertyId']);respond(get_client($client['id'])); }
    ...
    if (($method==='GET'||$method==='POST') && path_match('/public/client-form/{token}', $path, $params)) {
        $token=$params['token'];$stmt=db()->prepare('SELECT '.uuid_sql('q.id').' id,'.uuid_sql('q.client_id').' client_id,'.uuid_sql('q.property_id').' property_id,q.status,q.expires_at,c.first_name,p.name property_name FROM client_qr q JOIN clients c ON c.id=q.client_id JOIN properties p ON p.id=q.property_id WHERE q.token=? AND q.is_active=1 LIMIT 1');$stmt->execute([uuid_bin($token)]);$qr=$stmt->fetch();if(!$qr)fail('Linkul este invalid sau a fost regenerat.',404);if($qr['expires_at']&&strtotime($qr['expires_at'])<time())fail('Linkul a expirat.',410);
        if($method==='GET'){if($qr['status']!=='USED')db()->prepare('UPDATE client_qr SET opened_at=COALESCE(opened_at,?),updated_at=? WHERE id=?')->execute([now_utc(),now_utc(),uuid_bin($qr['id'])]);respond(['clientFirstName'=>$qr['first_name'],'propertyName'=>$qr['property_name'],'expiresAt'=>iso_date($qr['expires_at']),'used'=>$qr['status']==='USED']);}
        $body=json_body();if(strlen(trim((string)($body['fullName']??'')))<3||strlen(preg_replace('/\D/','',(string)($body['phone']??'')))<9||strlen(trim((string)($body['problem']??'')))<10)fail('Completează numele, telefonul și problema.',422);if(empty($body['gdpr'])||empty($body['accuracy'])||empty($body['terms']))fail('Acordurile sunt obligatorii.',422);$pdo=db();$pdo->beginTransaction();try{$now=now_utc();$intakeId=uuid_v4();$pdo->prepare('INSERT INTO client_intakes (id,client_id,qr_id,property_id,payload,submitted_at,is_active) VALUES (?,?,?,?,?,?,1) ON DUPLICATE KEY UPDATE payload=VALUES(payload),submitted_at=VALUES(submitted_at),is_active=1')->execute([uuid_bin($intakeId),uuid_bin($qr['client_id']),uuid_bin($qr['id']),uuid_bin($qr['property_id']),json_encode($body,JSON_UNESCAPED_UNICODE),$now]);$pdo->prepare("UPDATE client_qr SET status='USED',used_at=?,updated_at=? WHERE id=?")->execute([$now,$now,uuid_bin($qr['id'])]);$pdo->prepare("UPDATE clients SET first_name=?,last_name=?,phone=?,email=?,address=?,city=?,county=?,status='REVIEW_REQUIRED',updated_at=? WHERE id=?")->execute([explode(' ',trim($body['fullName']),2)[0],explode(' ',trim($body['fullName']),2)[1]??'',trim($body['phone']),$body['email']??null,$body['address']??null,$body['city']??null,$body['county']??null,$now,uuid_bin($qr['client_id'])]);$requestId=uuid_v4();$pdo->prepare("INSERT INTO service_requests (id,property_id,client_id,intake_id,status,created_at,updated_at) VALUES (?,?,?,?, 'NEW',?,?)")->execute([uuid_bin($requestId),uuid_bin($qr['property_id']),uuid_bin($qr['client_id']),uuid_bin($intakeId),$now,$now]);$pdo->prepare("INSERT INTO notifications (id,user_id,property_id,title,message,type,created_at) SELECT ?,up.user_id,?,'Solicitare nouă','Un client a completat formularul QR.','INFO',? FROM user_properties up WHERE up.property_id=?")->execute([uuid_bin(uuid_v4()),uuid_bin($qr['property_id']),$now,uuid_bin($qr['property_id'])]);$pdo->commit();audit_log('PUBLIC_FORM_SUBMITTED','qr','Clientul a completat formularul public','Client',$qr['client_id'],$qr['property_id'],null,['requestId'=>$requestId],[]);respond(['submitted'=>true,'requestId'=>$requestId],201);}catch(Throwable$e){$pdo->rollBack();throw$e;}
    }
    if ($method==='POST'&&$path==='/qr/resolve') { $user=require_permission('qr.scan');$body=json_body();$raw=(string)($body['data']??'');if(!preg_match('/([0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-5][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12})/',$raw,$match))fail('Cod QR G-Shop invalid.',422);$stmt=db()->prepare('SELECT '.uuid_sql('q.id').' id,'.uuid_sql('q.client_id').' client_id,'.uuid_sql('q.property_id').' property_id,q.status,q.expires_at,c.first_name,c.last_name FROM client_qr q JOIN clients c ON c.id=q.client_id WHERE q.token=? AND q.is_active=1 LIMIT 1');$stmt->execute([uuid_bin($match[1])]);$qr=$stmt->fetch();if(!$qr)fail('Codul este invalid sau a fost regenerat.',404);$valid=!$qr['expires_at']||strtotime($qr['expires_at'])>=time();$action=(string)($body['action']??'OPEN_PROFILE');if(!in_array($action,['OPEN_PROFILE','CHECK_IN','DROP_OFF','PICK_UP'],true))$action='OPEN_PROFILE';$propertyId=(string)($body['propertyId']??$qr['property_id']);ensure_property($propertyId,$user);db()->prepare('INSERT INTO qr_scan_logs (id,qr_id,client_id,property_id,scanned_by,action,device,status,created_at) VALUES (?,?,?,?,?,?,?,?,?)')->execute([uuid_bin(uuid_v4()),uuid_bin($qr['id']),uuid_bin($qr['client_id']),uuid_bin($qr['property_id']),uuid_bin($user['id']),$action,substr((string)($body['device']??''),0,100),$valid?'VALID':'EXPIRED',now_utc()]);if(!$valid)fail('Codul QR a expirat.',410);audit_log('QR_SCANNED','qr','QR scanat: '.$action,'ClientQR',$qr['id'],$qr['property_id'],null,['action'=>$action,'device'=>$body['device']??null],$user);respond(['clientId'=>$qr['client_id'],'clientName'=>$qr['first_name'].' '.$qr['last_name']]); }
    ...
    if ($method==='POST'&&$path==='/service-sheets') {
        ...
        $collaboratorId=!empty($client['collaboratorId'])?(string)$client['collaboratorId']:null;$commissionType=$client['commissionType']??null;$commissionRate=$client['commissionValue']??null;$commissionValue=null;
        if($collaboratorId!==null){
            collaborator_for_property($collaboratorId,$propertyId);
            if(in_array($commissionType,['PERCENT_NET','FIXED'],true)&&$commissionRate!==null)$commissionValue=commission_amount($netValue,$commissionType,(float)$commissionRate);
        }
        $id=uuid_v4();$commissionId=$commissionValue!==null?uuid_v4():null;$now=now_utc();$pdo=db();$pdo->beginTransaction();
        try{
            ...
            if($commissionId!==null){
                $pdo->prepare("INSERT INTO commissions (id,collaborator_id,client_id,service_sheet_id,property_id,total_value,direct_costs,net_value,type,rate_or_amount,commission_value,status,is_active,created_at,updated_at,created_by,updated_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,'APPROVED',1,?,?,?,?)")->execute([uuid_bin($commissionId),uuid_bin($collaboratorId),uuid_bin($clientId),uuid_bin($id),uuid_bin($propertyId),$totalCost,$directCosts,$netValue,$commissionType,(float)$commissionRate,$commissionValue,$now,$now,uuid_bin($user['id']),uuid_bin($user['id'])]);
            }
            $pdo->commit();
        }catch(Throwable$e){$pdo->rollBack();throw$e;}
        audit_log('SERVICE_SHEET_CREATED','service_sheets','Fișă creată: '.$number,'ServiceSheet',$id,$propertyId,null,$body,$user);
        if($commissionId!==null)audit_log('COMMISSION_CREATED','commissions','Comision aprobat automat pentru fișa '.$number,'Commission',$commissionId,$propertyId,null,['clientId'=>$clientId,'serviceSheetId'=>$id,'collaboratorId'=>$collaboratorId,'status'=>'APPROVED','commissionValue'=>$commissionValue],$user);
    ...
    if ($method==='PUT'&&path_match('/service-sheets/{id}',$path,$params)) {
        ...
        try{
            ...
            if($financeChanged){
                $commissionUpdate=$pdo->prepare("UPDATE commissions SET total_value=?,direct_costs=?,net_value=?,commission_value=ROUND(CASE WHEN type='PERCENT_NET' THEN ?*rate_or_amount/100 ELSE rate_or_amount END,2),updated_at=?,updated_by=? WHERE service_sheet_id=? AND is_active=1 AND status NOT IN ('PAID','CANCELLED')");
                $commissionUpdate->execute([$after['totalCost'],$after['directCosts'],$after['netValue'],$after['netValue'],$now,uuid_bin($user['id']),uuid_bin($params['id'])]);$recalculated=$commissionUpdate->rowCount();
                if($recalculated>0){$amountStmt=$pdo->prepare("SELECT commission_value FROM commissions WHERE service_sheet_id=? AND is_active=1 AND status NOT IN ('PAID','CANCELLED') ORDER BY created_at DESC LIMIT 1");$amountStmt->execute([uuid_bin($params['id'])]);$newCommission=$amountStmt->fetchColumn();$pdo->prepare('UPDATE service_sheets SET collaborator_commission=? WHERE id=?')->execute([$newCommission,uuid_bin($params['id'])]);}
            }
            $pdo->commit();
        }catch(Throwable$e){$pdo->rollBack();throw$e;}
    ...
    if ($method==='GET'&&$path==='/reports') {
        ...
        $metrics=['clientsTotal'=>$clients,'clientsNew'=>(int)$scalar('SELECT COUNT(*) FROM clients WHERE property_id=? AND is_active=1 AND created_at>=DATE_SUB(UTC_TIMESTAMP(),INTERVAL 30 DAY)',[$p]),'serviceSheetsOpen'=>(int)$scalar("SELECT COUNT(*) FROM service_sheets WHERE property_id=? AND is_active=1 AND status IN ('NEW','WAITING','VERIFYING','IN_PROGRESS','WAITING_PARTS')",[$p]),'serviceSheetsInProgress'=>(int)$scalar("SELECT COUNT(*) FROM service_sheets WHERE property_id=? AND is_active=1 AND status='IN_PROGRESS'",[$p]),'serviceSheetsCompleted'=>(int)$scalar("SELECT COUNT(*) FROM service_sheets WHERE property_id=? AND is_active=1 AND status IN ('COMPLETED','DELIVERED')",[$p]),'usersActive'=>(int)$scalar('SELECT COUNT(*) FROM user_properties up JOIN users u ON u.id=up.user_id WHERE up.property_id=? AND u.is_active=1',[$p]),'collaboratorsActive'=>(int)$scalar('SELECT COUNT(*) FROM collaborator_properties cp JOIN collaborators c ON c.id=cp.collaborator_id WHERE cp.property_id=? AND c.is_active=1',[$p]),'qrGenerated'=>$generated,'qrUsed'=>$used,'qrUnused'=>max(0,$clients-$generated-$used),'estimatedRevenue'=>$revenue,'collaboratorCommissions'=>$commissionTotal,'collaboratorPayments'=>$payments];
