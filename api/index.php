...
function map_sales_sheet(array $row, bool $includePaths=false): array {
    ...
    $item['paymentStatus']=in_array(($item['paymentStatus']??''),['UNPAID','PAID'],true)?$item['paymentStatus']:'UNPAID';$item['receivedAmount']=round($item['paymentStatus']==='PAID'?$item['totalPrice']:min($item['advancePaid'],$item['totalPrice']),2);$item['remainingDue']=round(max(0,$item['totalPrice']-$item['receivedAmount']),2);$item['gshopNet']=round($item['totalPrice']-$item['expenseTotal'],2);
...
function validated_sales_sheet_payload(array $body): array {
    $quantityInput=$body['quantity']??1;$quantity=(is_string($quantityInput)&&trim($quantityInput)==='')||$quantityInput===null||(is_numeric($quantityInput)&&(float)$quantityInput<=0)?1.0:sales_sheet_number_value($quantityInput,'Cantitatea',9999,true);$unit=sales_sheet_number_value($body['productUnitPrice']??0,'Prețul produsului',999999999.99);$delivery=sales_sheet_number_value($body['deliveryPrice']??0,'Prețul livrării',999999999.99);$advance=sales_sheet_number_value($body['advancePaid']??0,'Banii încasați',999999999.99);
    $productPrice=round($quantity*$unit,2);$total=round($productPrice+$delivery,2);if($advance>$total)fail('Banii încasați nu pot depăși prețul total.',422);$paymentStatus=strtoupper(trim((string)($body['paymentStatus']??'UNPAID')));if(!in_array($paymentStatus,['UNPAID','PAID'],true))fail('Statusul plății nu este valid.',422);$received=$paymentStatus==='PAID'?$total:$advance;$remaining=round($total-$received,2);
    ...
    return [
        ...
        'paymentMethod'=>$payment,'deliveryMode'=>$deliveryMode,'productUnitPrice'=>$unit,'productPrice'=>$productPrice,'deliveryPrice'=>$delivery,'totalPrice'=>$total,'advancePaid'=>$advance,'remainingDue'=>$remaining,'paymentStatus'=>$paymentStatus,'dueAt'=>sales_sheet_db_date($body['dueAt']??null,'Data scadenței'),'currencyCode'=>$currency,'notes'=>sales_sheet_text($body['notes']??null,'Observațiile',1600),'expenses'=>json_encode($expenses,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),'expenseTotal'=>$expenseTotal,'gshopNet'=>$gshopNet,
    ];
}
...
function generate_sales_sheet_record(string $sheetId,array $user): array {
    $row=sales_sheet_row($sheetId);$previousPath=(string)($row['file_path']??'');$sheet=map_sales_sheet($row,true);$company=json_decode((string)($row['company_snapshot']??''),true);if(!is_array($company))$company=[];require_once __DIR__.'/src/sales_sheet_pdf.php';$rendered=generate_sales_sheet_pdf($sheet,$company,$row['signature_path']??null,$company['stampPath']??null);$generatedAt=gmdate('Y-m-d H:i:s',strtotime((string)$rendered['generatedAt']));
    db()->prepare('UPDATE sales_sheets SET file_path=?,file_sha256=?,generated_at=?,updated_at=?,updated_by=? WHERE id=?')->execute([$rendered['filePath'],$rendered['sha256'],$generatedAt,now_utc(),uuid_bin($user['id']),uuid_bin($sheetId)]);if($previousPath!==''&&$previousPath!==$rendered['filePath']){$candidate=realpath(__DIR__.'/'.ltrim($previousPath,'/'));$root=realpath(__DIR__.'/uploads/sales-sheets');if($candidate&&$root&&str_starts_with($candidate,$root.DIRECTORY_SEPARATOR)&&is_file($candidate))@unlink($candidate);}refresh_existing_sales_documents_safe($sheetId,$user);return get_sales_sheet($sheetId);
}
...
try {
    ...
    if($method==='PUT'&&path_match('/sales-sheets/{id}',$path,$params)){
        $user=require_permission('sales_sheets.update');$body=json_body();$before=get_sales_sheet($params['id']);ensure_property($before['propertyId'],$user);if(!user_has_permission($user,'financials.view')){if(!empty($body['expenses']))fail('Nu ai permisiunea de a modifica cheltuieli interne.',403);$body['expenses']=$before['expenses']??[];}$values=validated_sales_sheet_payload($body);$columns=['document_at','customer_name','customer_phone','customer_email','delivery_address','customer_notes','product_name','product_code','serial_number','quantity','warranty','payment_method','delivery_mode','product_unit_price','product_price','delivery_price','total_price','advance_paid','remaining_due','payment_status','due_at','currency_code','notes','expenses','expense_total','gshop_net'];$args=array_values($values);$assignments=array_map(fn($column)=>$column.'=?',$columns);$args[] = now_utc();$args[]=uuid_bin($user['id']);$args[]=uuid_bin($before['id']);db()->prepare('UPDATE sales_sheets SET '.implode(',',$assignments).',updated_at=?,updated_by=? WHERE id=?')->execute($args);$after=generate_sales_sheet_record($before['id'],$user);audit_log('SALES_SHEET_UPDATED','sales_sheets','Fișă de vânzare actualizată: '.$after['number'],'SalesSheet',$after['id'],$after['propertyId'],$before,$after,$user);respond(sales_sheet_for_user($after,$user));
    }
    if($method==='PUT'&&path_match('/sales-sheets/{id}/expenses',$path,$params)){
        $user=require_permission('sales_sheets.update');require_permission('financials.view');$before=get_sales_sheet($params['id']);ensure_property($before['propertyId'],$user);$expenses=validated_sales_expenses(json_body()['expenses']??[]);$expenseTotal=sales_expense_total($expenses);$gshopNet=round((float)$before['totalPrice']-$expenseTotal,2);$now=now_utc();db()->prepare('UPDATE sales_sheets SET expenses=?,expense_total=?,gshop_net=?,updated_at=?,updated_by=? WHERE id=?')->execute([json_encode($expenses,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),$expenseTotal,$gshopNet,$now,uuid_bin($user['id']),uuid_bin($before['id'])]);$after=get_sales_sheet($before['id']);audit_log('SALES_SHEET_EXPENSES_UPDATED','sales_sheets','Cheltuieli actualizate pe '.$after['number'],'SalesSheet',$after['id'],$after['propertyId'],['expenses'=>$before['expenses']??[],'expenseTotal'=>$before['expenseTotal']??0],['expenses'=>$after['expenses'],'expenseTotal'=>$after['expenseTotal']],$user);respond($after);
    }
    if($method==='PUT'&&path_match('/sales-sheets/{id}/payment',$path,$params)){
        $user=require_permission('sales_sheets.update');$before=get_sales_sheet($params['id']);ensure_property($before['propertyId'],$user);$body=json_body();$paymentStatus=strtoupper(trim((string)($body['paymentStatus']??$before['paymentStatus']??'UNPAID')));if(!in_array($paymentStatus,['UNPAID','PAID'],true))fail('Statusul plății nu este valid.',422);$advance=sales_sheet_number_value($body['advancePaid']??$before['advancePaid']??0,'Banii încasați',999999999.99);$total=(float)$before['totalPrice'];if($advance>$total)fail('Banii încasați nu pot depăși prețul total.',422);$received=$paymentStatus==='PAID'?$total:$advance;$remaining=round(max(0,$total-$received),2);$gshopNet=round($total-(float)($before['expenseTotal']??0),2);$now=now_utc();db()->prepare('UPDATE sales_sheets SET advance_paid=?,remaining_due=?,payment_status=?,gshop_net=?,updated_at=?,updated_by=? WHERE id=?')->execute([$advance,$remaining,$paymentStatus,$gshopNet,$now,uuid_bin($user['id']),uuid_bin($before['id'])]);$after=generate_sales_sheet_record($before['id'],$user);audit_log('SALES_SHEET_PAYMENT_UPDATED','sales_sheets','Plată actualizată pe '.$after['number'],'SalesSheet',$after['id'],$after['propertyId'],['paymentStatus'=>$before['paymentStatus'],'receivedAmount'=>$before['receivedAmount']],['paymentStatus'=>$after['paymentStatus'],'receivedAmount'=>$after['receivedAmount']],$user);respond(sales_sheet_for_user($after,$user));
    }
