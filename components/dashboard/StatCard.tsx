...
export function StatCard({ label, value, icon, color, helper, style }: { label: string; value: string | number; icon: ComponentProps<typeof Ionicons>['name']; color: string; helper?: string; style?: StyleProp<ViewStyle> }) {
  const { colors, isDark } = useAppTheme();
  return <Card elevated style={[styles.card, { borderColor: isDark ? `${color}38` : colors.border }, style]}>
    ...
    <View style={styles.topRow}>
      ...
      {helper ? <View style={[styles.helper, { backgroundColor: isDark ? `${color}20` : `${color}10` }]}><Ionicons name="trending-up" size={12} color={color} /><AppText variant="caption" style={{ color, fontWeight: '800' }}>{helper}</AppText></View> : <View style={[styles.statusDot, { backgroundColor: color }]} />}
    </View>
