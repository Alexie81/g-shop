...
export function ServiceSheetStatus({ status }: { status: Status }) { const { isDark } = useAppTheme(); const color = tones[status]; return <View style={[styles.badge, { backgroundColor: isDark ? `${color}28` : `${color}14` }]}><View style={[styles.dot, { backgroundColor: color }]} /><AppText variant="caption" style={{ color, fontWeight: '800' }}>{SERVICE_STATUS_LABELS[status]}</AppText></View>; }
const styles = StyleSheet.create({ badge: { alignSelf: 'flex-start', borderRadius: radius.pill, paddingVertical: 6, paddingHorizontal: spacing.sm, flexDirection: 'row', alignItems: 'center', gap: 6 }, dot: { width: 6, height: 6, borderRadius: 3 } });
