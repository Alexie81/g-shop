...
import { GenerateSalesDocumentInput, SalesDocument, SalesDocumentType, SalesExpenseLine, SalesSheet, ServiceDocumentItem } from '@/types';
...
type DraftItem = ServiceDocumentItem & { key: string };
...
function DocumentItemsEditor({ title, description, items, onChange, currencyCode, importExpenses }: { title: string; description?: string; items: DraftItem[]; onChange: (items: DraftItem[]) => void; currencyCode: string; importExpenses?: SalesExpenseLine[] }) {
  const { colors } = useAppTheme();
